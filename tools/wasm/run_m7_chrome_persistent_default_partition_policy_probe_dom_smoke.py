...
"""Run the narrow live persistent-default-partition policy probe.

This runner admits only the fresh source-selected
``chrome_wasm_m7_persistent_default_partition_policy_probe`` artifact.  It
starts one isolated host document with exactly this native argument:

  --wasm-persistent-default-partition-policy-probe=

The host accepts only the fixed stderr sequence
``DEFAULT_CONFIG_DEFAULT_NOT_IN_MEMORY``, ``FENCE_OK``, and
``POLICY_PROBE_COMPLETE``.  It also requires the native process-exit import,
Emscripten's normal ``onExit(0)`` acknowledgement, and a server receipt
acknowledgement before this runner can report success.

This is intentionally a policy/configuration witness.  It does not construct
or prove a real StoragePartition, profile persistence, a fresh-document
reload, crash recovery, or M7 completion.

Build the dedicated artifact first:

  buildtools/linux64/gn gen \
      out/wasm-chrome-m7-persistent-default-partition-policy-probe \
      --args='import("//out/wasm-chrome-m6/args.gn") \
      enable_chromium_wasm_m7_persistent_default_partition_policy_probe=true' \
      --fail-on-unused-args
  autoninja -C out/wasm-chrome-m7-persistent-default-partition-policy-probe \
      chrome_wasm
"""
...
SENTINEL = "CHROMIUM_WASM_M7_PERSISTENT_DEFAULT_PARTITION_POLICY_PROBE_DOM"
CASE = "chrome_persistent_default_partition_policy_probe_m7"
SCOPE = (
    "one-fresh-source-selected-chrome-wasm-policy-configuration-observation-"
    "only-no-storagepartition-or-persistence-claim"
)
PRODUCT_MODULE_NAME = "chrome_wasm_m7_persistent_default_partition_policy_probe"
...
EXACT_EMPTY_PROBE_SWITCH = "--wasm-persistent-default-partition-policy-probe="
HOST_ROOT = "/__m7_persistent_default_partition_policy_probe__"
HOST_HTML_NAME = "chrome_wasm_persistent_default_partition_policy_probe_smoke.html"
HOST_JS_NAME = "chrome_wasm_persistent_default_partition_policy_probe_smoke.js"
...
M7_POLICY_MARKER_PREFIX = "CHROMIUM_WASM_M7_PERSISTENT_DEFAULT_PARTITION_POLICY:"
EXPECTED_MARKERS = (
    M7_POLICY_MARKER_PREFIX + "DEFAULT_CONFIG_DEFAULT_NOT_IN_MEMORY",
    M7_POLICY_MARKER_PREFIX + "FENCE_OK",
    M7_POLICY_MARKER_PREFIX + "POLICY_PROBE_COMPLETE",
)
M7_POLICY_FAIL_PREFIX = M7_POLICY_MARKER_PREFIX + "FAIL stage="
...
POLICY_GN_ENABLE_ASSIGNMENT_RE = re.compile(
    r"^[ \t]*enable_chromium_wasm_m7_persistent_default_partition_policy_probe"
    r"[ \t]*=[ \t]*(true|false)[ \t]*(?:#.*)?$",
    re.MULTILINE,
)
...
_RUN_FIELDS = frozenset(
    (
        ...
        "normalProcessExitAndAckReceived",
        "onExitCount",
...
_RESULT_FIELDS = frozenset(
    (
        ...
        "freshSourceSelectedPolicyArtifactProven",
        "hostBoundary",
        "m7GateComplete",
        "normalProcessExitAndAckProven",
        "policyFenceProven",
...
class ProtocolStateError(M0Error):
    """A one-use policy-probe receipt state conflict."""


class PolicyProbeSession:
    """Stages one result and releases it only after its host acknowledgement."""

    def __init__(self, result_token: str):
        if not CAPABILITY_RE.fullmatch(result_token):
            raise M0Error("policy probe result capability is invalid")
    ...
    def stage_result(self, result_token: str, value: dict[str, Any]) -> bool:
        ...
        with self._lock:
            if self._result is not None:
                raise ProtocolStateError("policy probe result state conflict")
            self._result = value
            return True
    ...
    def acknowledge_result(self, result_token: str) -> dict[str, Any] | None:
        ...
        with self._lock:
            if self._result is None or self._acknowledged:
                raise ProtocolStateError("policy probe acknowledgement conflict")
            self._acknowledged = True
            return self._result


class PersistentDefaultPartitionPolicyProbeServer(ThreadingHTTPServer):
    """In-memory immutable source-selected policy-probe delivery."""


class PersistentDefaultPartitionPolicyProbeRequestHandler(BaseHTTPRequestHandler):
    server: PersistentDefaultPartitionPolicyProbeServer

    def log_message(self, _format: str, *_args: object) -> None:
        # Browser requests are intentionally captured only through the strict
        # protocol state, rather than copied to stdout.
        return
    ...
    def _conflict(self) -> None:
        self._send_bytes(
            ...
            b"policy probe endpoint state conflict\n",
        )
    ...
    def _post_result(self, result_token: str) -> None:
        ...
        if not accepted:
            self._not_found()
            return
        # The host must receive this acknowledgement before it can send the
        # separate receipt acknowledgement endpoint below.
    ...
    def _post_acknowledgement(self, result_token: str) -> None:
        ...
        if (
            ...
            self._send_bytes(
                HTTPStatus.BAD_REQUEST,
                "text/plain; charset=utf-8",
                b"invalid policy probe acknowledgement\n",
            )
            return
        try:
        ...
        try:
            ...
            # The one-use session remains terminal; do not admit a duplicate
            # result merely because the runner stopped receiving receipts.
            return
...
def _require_product_module_name(module_name: object, boundary: str) -> str:
    if not isinstance(module_name, str) or module_name != PRODUCT_MODULE_NAME:
        raise M0Error(f"policy probe {boundary} module is invalid")
    return module_name


def validate_m7_output_configuration(args_gn: bytes, out_dir: Path) -> None:
    """Accept only the dedicated source-selected policy-probe configuration."""

    if out_dir.name != DEFAULT_OUT_DIR.name:
        raise M0Error("policy probe runner requires its dedicated output directory")
    try:
        text = args_gn.decode("utf-8").replace("\r\n", "\n")
    except UnicodeDecodeError as exc:
        raise M0Error("policy probe args.gn is not UTF-8") from exc
    values = POLICY_GN_ENABLE_ASSIGNMENT_RE.findall(text)
    if values != ["true"]:
        raise M0Error("policy probe args.gn lacks its exact test opt-in")
    if any(
        value == "true"
        for assignment_re in INCOMPATIBLE_M7_ENABLE_ASSIGNMENT_RES
        for value in assignment_re.findall(text)
    ):
        raise M0Error("policy probe args.gn enables an incompatible M7 target")
...
def create_server(
    ...
    *,
...
) -> PersistentDefaultPartitionPolicyProbeServer:
    """Snapshot the exact policy artifact and its harness before serving it."""
    ...
    artifacts = snapshot_regular_files(
        ...
        description="persistent-default-partition policy probe artifacts",
    )
    args_gn = snapshot_regular_file(
        out_dir / "args.gn",
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="persistent-default-partition policy probe args.gn",
    )
    ...
    host_snapshots = snapshot_regular_files(
        ...
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="persistent-default-partition policy probe host resources",
    )
    runner_source = snapshot_regular_file(
        runner_source_path or Path(__file__),
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="persistent-default-partition policy probe runner source",
    )
    server = PersistentDefaultPartitionPolicyProbeServer(
        (host, port), PersistentDefaultPartitionPolicyProbeRequestHandler
    )
    ...
    server.session = PolicyProbeSession(result_token)
    return server
...
def artifact_identity(
    ...
    if server.module_name != PRODUCT_MODULE_NAME:
        raise M0Error("policy probe server module disagrees with its artifact")
    return {
...
def toolchain_manifest_versions(manifest: dict[str, Any]) -> dict[str, str]:
    try:
        ...
        raise M0Error("policy probe manifest lacks version metadata") from exc
    if not all(
        isinstance(value, str) and GIT_REVISION_RE.fullmatch(value)
        for value in versions.values()
    ):
        raise M0Error("policy probe manifest version metadata is invalid")
    return versions
...
def _require_exact_fields(
    ...
    if not isinstance(value, dict) or set(value) != fields:
        raise M0Error(f"policy probe {name} schema is invalid")
    return value
...
def _require_equal(value: dict[str, Any], field: str, expected: object) -> None:
    if not _exact_json_equal(value.get(field), expected):
        raise M0Error(f"policy probe result {field} is invalid")
...
def _validate_byte_identity(value: object, description: str) -> None:
    ...
    if (
        ...
        raise M0Error(f"policy probe {description} is invalid")
...
def _validate_artifact_identity(value: object, expected: dict[str, object]) -> None:
    ...
    if (
        ...
        raise M0Error("policy probe artifact identity is invalid")
    for field_name in ("build_config", "loader", "wasm"):
        _validate_byte_identity(artifact.get(field_name), f"artifact {field_name}")
    if artifact != expected:
        raise M0Error("policy probe artifact identity disagrees with snapshot")
...
def _validate_capture_harness_identity(
    ...
    if (
        ...
        raise M0Error("policy probe capture harness identity is invalid")
    for field_name in ("host_html", "host_js", "runner_source"):
        _validate_byte_identity(harness.get(field_name), f"capture {field_name}")
    if harness != expected:
        raise M0Error("policy probe capture harness disagrees with snapshot")
...
def _validate_run(value: object) -> None:
    ...
    if (
        ...
        not in ("resolved", "expected-normal-exit-status")
        ...
        or run.get("markerSource") != "stderr-only-fixed-policy-probe-grammar"
        or run.get("markers") != list(EXPECTED_MARKERS)
        or run.get("noFailMarkerObserved") is not True
        or run.get("normalProcessExitAndAckReceived") is not True
        ...
        raise M0Error("policy probe run receipt is invalid")
...
def _validate_bridge(value: object) -> None:
    ...
    if bridge != expected:
        raise M0Error("policy probe bridge receipt is invalid")
...
def _validate_quiescence(value: object) -> None:
    ...
    if (
        ...
        raise M0Error("policy probe quiescence receipt is invalid")
...
def _validate_host_boundary(value: object) -> None:
    ...
    if any(item is not False for item in boundary.values()):
        raise M0Error("policy probe host crossed a prohibited boundary")
...
def validate_result(
    ...
    *,
    ...
    """Accept only the fixed one-artifact policy/configuration receipt."""

    if _contains_prohibited_value(result, result_token):
        raise M0Error("policy probe receipt contains its result capability")
    ...
    for field, expected in {
        ...
        "defaultConfigDefaultNotInMemoryProven": True,
        "policyFenceProven": True,
        "policyProbeCompleteProven": True,
        "normalProcessExitAndAckProven": True,
...
def verify_server_delivery(server: PersistentDefaultPartitionPolicyProbeServer) -> None:
    """Check all delivered executable bytes are the startup snapshots."""
    ...
    for path, contents, content_type in expected:
        ...
        try:
            ...
            if response.status != HTTPStatus.OK:
                raise M0Error("policy probe snapshot request failed")
            ...
            for name, expected_value in _expected_headers(content_type).items():
                ...
                if actual != expected_value:
                    raise M0Error("policy probe snapshot response header is invalid")
            count, digest = _stream_response_digest(response)
            if count != len(contents) or digest != hashlib.sha256(contents).hexdigest():
                raise M0Error("policy probe snapshot body changed")
        finally:
            connection.close()
    ...
    while True:
        ...
        try:
            ...
            if browser.poll() is not None:
                raise M0Error(
                    "browser exited before policy probe receipt acknowledgement "
                    f"({len(browser_stderr)} browser stderr lines suppressed)"
                )


def policy_probe_summary() -> dict[str, object]:
    return {
        "actualStoragePartitionProven": False,
        "crashRecoveryProven": False,
        "freshDocumentReloadProven": False,
        "m7GateComplete": False,
        "normalProcessExitAndResultAckProven": True,
        "policyConfigurationOnlyProven": True,
        "profilePersistenceProven": False,
    }
...
def write_failure_diagnostics(
    ...
    *,
    ...
    """Write only bounded runner state, never capability-bearing details."""

    diagnostics_dir.mkdir(parents=True, exist_ok=True)
    path = diagnostics_dir / "persistent-default-partition-policy-probe-failure.json"
    payload = {
        "schema_version": 1,
...
def _stop_server(
    server: PersistentDefaultPartitionPolicyProbeServer | None,
    ...
    if server_thread_started and server_thread is not None:
        ...
        if server_thread.is_alive():
            raise M0Error("policy probe server did not stop")


def parse_policy_probe_timeout(value: str) -> float:
    try:
        timeout = float(value)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the one-artifact persistent-default-partition policy probe; "
            "this makes no StoragePartition or persistence claim."
        ),
        epilog=(
    ...
    parser.add_argument("--timeout", type=parse_policy_probe_timeout, default=120.0)
    ...
    server: PersistentDefaultPartitionPolicyProbeServer | None = None
    ...
    try:
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-persistent-default-policy-probe-server",
            daemon=True,
        )
        server_thread.start()
        ...
        outer_profile = tempfile.TemporaryDirectory(
            prefix="chromium-wasm-m7-persistent-default-policy-probe-"
        )
        ...
        browser_stderr_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-persistent-default-policy-probe-browser-stderr",
            daemon=True,
        )
