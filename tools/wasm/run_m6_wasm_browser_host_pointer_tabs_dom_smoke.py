...
"""Run a real trusted-DOM pointer tab flow through Chrome's Wasm host.
...
SENTINEL = "CHROMIUM_WASM_M6_HOST_POINTER_TABS_DOM"
CASE = "browser_host_pointer_tabs_m6"
SCOPE = "trusted-dom-pointer-ozone-aura-views-tab-flow"
...
HOST_ROOT = "/__m6_browser_host_pointer_tabs__"


class HostPointerTabsSmokeServer(ThreadingHTTPServer):
    daemon_threads = True
...
class HostPointerTabsSmokeRequestHandler(BaseHTTPRequestHandler):
    server: HostPointerTabsSmokeServer
    ...
    def do_GET(self) -> None:
        ...
        if path in (HOST_ROOT, f"{HOST_ROOT}/"):
            ...
            return
        if path == f"{HOST_ROOT}/chrome_wasm_browser_host_pointer_tabs_smoke_host.js":
            self._send_bytes(
                HTTPStatus.OK,
    ...
    def do_POST(self) -> None:
        ...
        if result is None:
            self._send_bytes(
                ...
                "text/plain; charset=utf-8",
                b"invalid host-pointer-tab result\n",
            )
            return
        with self.server.result_lock:
            if self.server.result_received:
                self._send_bytes(
                    HTTPStatus.CONFLICT,
                    "text/plain; charset=utf-8",
                    b"host-pointer-tab result already received\n",
                )
                return
            try:
                ...
                self._send_bytes(
                    HTTPStatus.CONFLICT,
                    "text/plain; charset=utf-8",
                    b"host-pointer-tab result queue is full\n",
                )
                return
...
def create_server(
    ...
    *,
    ...
    server.html_bytes = (
        host_dir / "chrome_wasm_browser_host_pointer_tabs_smoke.html"
    ).read_bytes()
    server.host_js_bytes = (
        host_dir / "chrome_wasm_browser_host_pointer_tabs_smoke_host.js"
    ).read_bytes()
...
def _require_equal(result: dict[str, Any], field: str, expected: object) -> None:
    if not browser_view_smoke._exact_json_value_equal(result.get(field), expected):
        raise M0Error(
            f"host-pointer-tab result {field} mismatch: expected {expected!r}, "
...
def _validate_target(value: object, name: str) -> dict[str, object]:
    if not isinstance(value, dict):
        raise M0Error(f"host-pointer-tab {name} target is missing")
    for field in ("x", "y"):
        coordinate = value.get(field)
        if type(coordinate) is not int or not 0 <= coordinate < MAX_FRAME_DIMENSION:
            raise M0Error(f"host-pointer-tab {name} target {field} is invalid")
...
def _validate_host_input(value: object) -> None:
    if not isinstance(value, dict):
        raise M0Error("host-pointer-tab result has no host-input evidence")
    for field in (
        "attached",
        "readyObserved",
        "insertedObserved",
    ...
    if not isinstance(records, list):
        raise M0Error("host-pointer-tab pointer records are missing")
    rejected = [record for record in records if isinstance(record, dict) and not record.get("accepted")]
    if rejected:
        raise M0Error("host-pointer-tab rejected an outer trusted pointer record")
    actions = [
        record
    ...
    for index, (event_type, target, expected_buttons) in enumerate(expected):
        ...
        for field, expected_value in {
            ...
            if record.get(field) != expected_value:
                raise M0Error(
                    f"host-pointer-tab action {index} {field} is invalid: "
...
def validate_result(result: dict[str, Any], *, expected_versions: dict[str, str]) -> None:
    ...
    if process_exit_code is not None and process_exit_code != 0:
        raise M0Error("host-pointer-tab bridge process exit disagrees with runtime")
    if result.get("versions") != expected_versions:
        raise M0Error("host-pointer-tab versions do not match the manifest")
    for field in ("fatalErrors", "windowErrors", "unhandledRejections"):
        if not isinstance(result.get(field), list) or result[field]:
            raise M0Error(f"host-pointer-tab {field} is not empty")
...
def _take_early_result(
    ...
    try:
        ...
        return
    raise M0Error(
        f"host-pointer-tab smoke finished before {stage}: "
...
def wait_for_state(
    ...
    expression = "globalThis.__chromiumWasmM6HostPointerTabsState || null"
    while time.monotonic() < deadline:
        ...
        time.sleep(0.05)
    raise M0Error(
        f"host-pointer-tab smoke did not reach {desired}: "
...
def click_target(client: Any, state: dict[str, object], field: str) -> None:
    ...
    if not isinstance(target, dict):
        raise M0Error(f"host-pointer-tab state lacks {field}")
    x = target.get("clientX")
    y = target.get("clientY")
    if not isinstance(x, (int, float)) or not isinstance(y, (int, float)):
        raise M0Error(f"host-pointer-tab {field} client coordinates are invalid")
    # This is a real Chrome DevTools physical mouse stream. It deliberately
    # does not evaluate page JavaScript or call any Wasm export directly.
...
def wait_for_result(
    ...
    while True:
        if browser.poll() is not None:
            raise M0Error(
                "host browser exited before host-pointer-tab result: "
                + "\n".join(browser_stderr)
            )
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            raise M0Error("host-pointer-tab smoke timeout: " + "\n".join(browser_stderr))
        try:
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = diagnostics_dir / "chrome-browser-host-pointer-tabs-m6-failure.json"
    payload = {
        "schema_version": 1,
        "runner": "run_m6_wasm_browser_host_pointer_tabs_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "stage": stage,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run trusted DOM pointer tab input through Chrome's Wasm host."
    )
    ...
    server: HostPointerTabsSmokeServer | None = None
    ...
    try:
        ...
        for suffix in (".js", ".wasm"):
            if not (out_dir / f"{args.module_name}{suffix}").is_file():
                raise M0Error(f"host-pointer-tab artifact is missing: {suffix}")
        stage = "load_manifest"
        ...
        context = print_context(
            "run_m6_wasm_browser_host_pointer_tabs_dom_smoke.py",
            manifest,
            case=CASE,
            scope=SCOPE,
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m6-host-pointer-tabs-server",
            daemon=True,
        )
        server_thread.start()
        url = smoke_url(
        ...
        profile = tempfile.TemporaryDirectory(prefix="chromium-wasm-m6-host-pointer-tabs-")
        ...
        stderr_thread = threading.Thread(
            ...
            name="chromium-wasm-m6-host-pointer-tabs-browser-stderr",
            daemon=True,
        )
        stderr_thread.start()
        ...
        stage = "dispatch_trusted_dom_close_tab"
        click_target(client, state, "closeTabTarget")
