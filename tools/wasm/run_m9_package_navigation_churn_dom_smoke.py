...
"""Exercise fixed native navigation churn from verified pre-release package bytes.
...
fixed native local-data navigation-churn lane. It does not read a raw build
output directory, alter the public package host, or claim that the public
package index exposes a navigation-churn interface.
...
if __package__:
    ...
    from . import run_m9_wasm_browser_navigation_churn_dom_smoke as navigation_churn
    ...
    import run_m9_wasm_browser_navigation_churn_dom_smoke as navigation_churn
...
SENTINEL = "CHROMIUM_WASM_M9_PACKAGE_NAVIGATION_CHURN_DOM"
...
SCOPE = (
    ...
    "same-instance-local-data-navigation-churn-only"
)
...
LIMITATIONS = (
    "does_not_exercise_the_public_package_index_or_release_host_navigation_churn_mode",
...
class PackageNavigationChurnSnapshot:
...
def _require_exact_fields(
    ...
    if type(value) is not dict or set(value) != expected:
        raise M0Error(f"package navigation-churn {description} schema is invalid")
    return value


def _byte_identity(contents: bytes) -> dict[str, object]:
    if type(contents) is not bytes or not contents:
        raise M0Error("package navigation-churn executable bytes are invalid")
    return {
        "bytes": len(contents),
...
def _validate_byte_identity(value: object, description: str) -> None:
    ...
    if type(identity.get("bytes")) is not int or identity["bytes"] < 1:
        raise M0Error(f"package navigation-churn {description} byte count is invalid")
    digest = identity.get("sha256")
    if type(digest) is not str or not SHA256_RE.fullmatch(digest):
        raise M0Error(f"package navigation-churn {description} SHA-256 is invalid")
...
def _validate_runtime_metadata(value: object) -> dict[str, object]:
    ...
    if (
        ...
        raise M0Error("package navigation-churn runtime metadata is invalid")
    version_sha256 = metadata.get("versionJsonSha256")
    if type(version_sha256) is not str or not SHA256_RE.fullmatch(version_sha256):
        raise M0Error("package navigation-churn VERSION.json identity is invalid")
    ...
    if (
        ...
        raise M0Error("package navigation-churn gate state schema is invalid")
    for name, expected in package_tool.EXPECTED_GATE_STATE.items():
        if type(gate_state.get(name)) is not bool or gate_state[name] is not expected:
            raise M0Error("package navigation-churn gate state is not the false-only contract")
    ...
    if (
        ...
        raise M0Error("package navigation-churn artifact source provenance is invalid")
    if (
        ...
        raise M0Error("package navigation-churn runtime build metadata is invalid")
    ...
    if not all(
        ...
        raise M0Error("package navigation-churn runtime versions are invalid")
    return metadata
...
def validate_package_artifact_identity(
    ...
    *,
    ...
    if artifact.get("artifact_delivery") != PACKAGE_ARTIFACT_DELIVERY:
        raise M0Error("package navigation-churn artifact delivery is invalid")
    if artifact.get("public_module_name") != PUBLIC_MODULE_NAME:
        raise M0Error("package navigation-churn public module name is invalid")
    if artifact.get("private_module_name") != PRIVATE_MODULE_NAME:
        raise M0Error("package navigation-churn private module name is invalid")
    ...
    if runtime_metadata is not None:
        ...
        if provenance != build["artifactSourceProvenance"]:
            raise M0Error(
                "package navigation-churn artifact provenance disagrees with VERSION.json"
            )
    if expected is not None and not _exact_json_value_equal(artifact, expected):
        raise M0Error("package navigation-churn artifact identity disagrees with expectation")
    return artifact


def capture_package_navigation_churn_snapshot(dist_dir: Path) -> PackageNavigationChurnSnapshot:
    ...
    if (
        ...
        raise M0Error("package navigation-churn private executable snapshot is invalid")
    build = metadata["build"]
    assert isinstance(build, dict)
    artifact_identity = {
    ...
    return PackageNavigationChurnSnapshot(
        artifact=artifact,
...
def create_package_navigation_churn_server(
    host: str,
    port: int,
    snapshot: PackageNavigationChurnSnapshot,
    token: str,
    result_queue: queue.Queue[dict[str, Any]],
    *,
    host_dir: Path | None = None,
    runner_source_path: Path | None = None,
) -> navigation_churn.NavigationChurnSmokeServer:
    """Serve only private aliases of the captured package executable bytes."""

    if not isinstance(snapshot, PackageNavigationChurnSnapshot):
        raise M0Error("package navigation-churn snapshot is invalid")
    validate_package_artifact_identity(
    ...
    return navigation_churn.create_server_from_artifacts(
        host,
        port,
        aliases,
        token,
...
def _require_alias_identity_matches_package(
    ...
    for name in ("loader", "wasm"):
        ...
        if not _exact_json_value_equal(alias, package):
            raise M0Error(
                "package navigation-churn private alias does not match packaged "
                f"{name} bytes"
            )


def validate_package_navigation_churn_result(
    value: object,
    *,
    expected_snapshot: PackageNavigationChurnSnapshot,
    expected_alias_identity: dict[str, object],
    expected_capture_harness_identity: dict[str, object],
) -> dict[str, object]:
    """Validate package binding plus all existing native navigation evidence."""

    result = _require_exact_fields(value, _RESULT_FIELDS, "result")
    if result.get("scope") != SCOPE:
        raise M0Error("package navigation-churn result scope is invalid")
    if type(result.get("m9GateComplete")) is not bool or result["m9GateComplete"]:
        raise M0Error("package navigation-churn result must not complete M9")
    if result.get("releaseStatus") != package_tool.RELEASE_STATUS:
        raise M0Error("package navigation-churn result release status is invalid")
    validate_package_artifact_identity(
    ...
    if not _exact_json_value_equal(
        ...
        raise M0Error("package navigation-churn runtime metadata disagrees with snapshot")
    _require_alias_identity_matches_package(
        expected_alias_identity, expected_snapshot.artifact_identity
    )
    runtime_result = result.get("navigationChurn")
    if type(runtime_result) is not dict:
        raise M0Error("package navigation-churn runtime result is invalid")
    navigation_churn.validate_result(
        runtime_result,
...
def package_navigation_churn_result(
    runtime_result: dict[str, Any], snapshot: PackageNavigationChurnSnapshot
) -> dict[str, object]:
    ...
    return {
        ...
        "scope": SCOPE,
        "navigationChurn": runtime_result,
    }
...
def _cleanup_server(
    *,
    server: navigation_churn.NavigationChurnSmokeServer | None,
        ...
        if server_thread_started:
            cleanup_error = _run_cleanup_action(
                ...
                lambda: shutdown_server_bounded(
                    server, timeout=1, description="M9 package navigation-churn server"
                ),
            )
    ...
    if server_thread_started and server_thread is not None:
        ...
        if server_thread.is_alive() and cleanup_error is None:
            cleanup_error = M0Error("M9 package navigation-churn server did not stop")
    if server is not None:
        cleanup_error = _run_cleanup_action(
            cleanup_error,
            lambda: server.join_request_handlers(
                timeout=1, description="M9 package navigation-churn server"
            ),
        )
    return cleanup_error
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = diagnostics_dir / "chrome-browser-package-navigation-churn-m9-failure.json"
    payload = {
        "schema_version": 1,
        "runner": "run_m9_package_navigation_churn_dom_smoke.py",
        "scope": SCOPE,
        "stage": stage,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run fixed native local-data navigation churn from verified pre-release "
            "package bytes."
        )
    )
    ...
    diagnostics_dir = args.diagnostics_dir or (
        Path(tempfile.gettempdir()) / "chromium-wasm-m9-package-navigation-churn-diagnostics"
    )
    if not diagnostics_dir.is_absolute():
        diagnostics_dir = REPO_ROOT / diagnostics_dir

    server: navigation_churn.NavigationChurnSmokeServer | None = None
    ...
    package_snapshot: PackageNavigationChurnSnapshot | None = None
    ...
    try:
        package_snapshot = capture_package_navigation_churn_snapshot(dist_dir)
        versions = _runtime_versions(package_snapshot.runtime_metadata)
        token = secrets.token_urlsafe(24)
        result_queue: queue.Queue[dict[str, Any]] = queue.Queue(maxsize=1)

        stage = "create_server"
        server = create_package_navigation_churn_server(
            "127.0.0.1", 0, package_snapshot, token, result_queue
        )
        alias_identity = navigation_churn.artifact_identity(
            server, module_name=PRIVATE_MODULE_NAME
        )
        capture_harness = navigation_churn.capture_harness_identity(server)
        _require_alias_identity_matches_package(
            alias_identity, package_snapshot.artifact_identity
        )
        navigation_churn.verify_required_exports(server.artifacts[f"{PRIVATE_MODULE_NAME}.js"])
        context = {
            ...
            "runtime_arguments": [navigation_churn.SWITCH],
            "scope": SCOPE,
            "script": "run_m9_package_navigation_churn_dom_smoke.py",
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m9-package-navigation-churn-server",
            daemon=True,
        )
        server_thread.start()
        server_thread_started = True
        url = navigation_churn.smoke_url(
            server,
            token,
            versions,
        ...
        profile = tempfile.TemporaryDirectory(
            prefix="chromium-wasm-m9-package-navigation-churn-"
        )
        ...
        stderr_reader = BrowserStderrReader(
            browser.stderr,
            browser_stderr,
            name="chromium-wasm-m9-package-navigation-churn-browser-stderr",
            thread_factory=threading.Thread,
        )
        stderr_reader.start()
        deadline = time.monotonic() + args.timeout
        stage = "wait_for_result"
        runtime_result = navigation_churn.wait_for_result(
            browser, browser_stderr, result_queue, deadline
        )
        result = package_navigation_churn_result(runtime_result, package_snapshot)
        stage = "validate_result"
        validate_package_navigation_churn_result(
            result,
    ...
    # The legacy navigation-churn runner imports ``m0_common`` as a top-level
    # module.
    ...
    if reported_error is not None:
        ...
        return 1
    if result is None:
        raise RuntimeError("package navigation-churn completed without a result")
    print(
