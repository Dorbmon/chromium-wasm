...
"""Focused contracts for package-byte-only native navigation-churn evidence."""
...
import package
import run_m9_package_navigation_churn_dom_smoke as runner
...
class M9PackageNavigationChurnDomSmokeTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def _snapshot(self) -> runner.PackageNavigationChurnSnapshot:
        return runner.capture_package_navigation_churn_snapshot(self.dist_dir)

    def _host_dir(self) -> Path:
        host_dir = self.root / "navigation-churn-host"
        host_dir.mkdir()
        (host_dir / "chrome_wasm_browser_navigation_churn_smoke.html").write_text(
            "<html>test host</html>\n", encoding="utf-8"
        )
        (
    ...
    def test_server_serves_only_private_in_memory_aliases_of_package_bytes(self) -> None:
        ...
        runner_source = self.root / "package-navigation-churn-runner.py"
        runner_source.write_text("# runner snapshot\n", encoding="utf-8")
        server = runner.create_package_navigation_churn_server(
            "127.0.0.1",
            0,
            snapshot,
            "token",
    ...
    def test_server_aliases_keep_wasm_mime_and_required_isolation_headers(self) -> None:
        ...
        try:
            ...
            with urlopen(
                f"http://{host}:{port}{navigation_churn.HOST_ROOT}/artifacts/chrome_wasm.js",
                timeout=5,
            ) as response:
            ...
            with urlopen(
                f"http://{host}:{port}{navigation_churn.HOST_ROOT}/artifacts/chrome_wasm.wasm",
                timeout=5,
            ) as response:
            ...
            with self.assertRaises(HTTPError) as error:
                urlopen(
                    f"http://{host}:{port}{navigation_churn.HOST_ROOT}/artifacts/"
                    "chromium-wasm.js",
                    timeout=5,
                )
            ...
            server.join_request_handlers(
                timeout=5, description="package navigation-churn test server"
            )
    ...
    def test_result_binds_package_metadata_and_private_aliases_before_child_validation(
        ...
        snapshot = self._snapshot()
        alias_identity = {
            "artifact_delivery": navigation_churn.ARTIFACT_DELIVERY,
            "artifact_source_provenance": navigation_churn.ARTIFACT_SOURCE_PROVENANCE,
        ...
        harness = {
            ...
            "source_snapshot_provenance": navigation_churn.SOURCE_SNAPSHOT_PROVENANCE,
            "version_provenance": navigation_churn.VERSION_PROVENANCE,
        }
        child = {"opaque": "existing-navigation-churn-result"}
        result = runner.package_navigation_churn_result(child, snapshot)

        with mock.patch.object(runner.navigation_churn, "validate_result") as validate_child:
            self.assertEqual(
                result,
                runner.validate_package_navigation_churn_result(
                    result,
        ...
        for mutate, expression in mutations:
            with self.subTest(expression=expression):
                ...
                mutate(invalid)
                with mock.patch.object(runner.navigation_churn, "validate_result"):
    ...
    def test_runner_accepts_only_a_package_directory_and_never_selects_raw_output(self) -> None:
        runner_source = source("tools/wasm/run_m9_package_navigation_churn_dom_smoke.py")
        ...
        self.assertNotIn("pointer-abi-rejection-seed", runner_source)
        self.assertNotIn("wait_for_page_client", runner_source)
    ...
    def test_main_rejects_an_out_dir_argument_before_any_package_capture(self) -> None:
        ...
        try:
            with (
                mock.patch.object(runner, "capture_package_navigation_churn_snapshot") as capture,
                mock.patch.object(
                    sys,
                    "argv",
                    [
                        "package-navigation-churn",
                        "--dist-dir",
                        str(self.dist_dir),
                        "--out-dir",
                        str(self.out_dir),
    ...
    def test_package_mode_normalizes_shared_navigation_churn_m0_error(self) -> None:
        package_runner = importlib.import_module(
            "tools.wasm.run_m9_package_navigation_churn_dom_smoke"
        )
        package_navigation_churn = importlib.import_module(
            "tools.wasm.run_m9_wasm_browser_navigation_churn_dom_smoke"
        )
        self.assertIs(package_navigation_churn, package_runner.navigation_churn)
        self.assertIsNot(package_runner.M0Error, package_navigation_churn.M0Error)
        stderr = io.StringIO()
        with (
            mock.patch.object(
                package_runner,
                "capture_package_navigation_churn_snapshot",
                side_effect=package_navigation_churn.M0Error("package-mode shared failure"),
            ),
            mock.patch.object(
                package_runner,
                "write_failure_diagnostics",
                return_value=Path("/tmp/package-navigation-churn-diagnostics.json"),
            ),
            mock.patch.object(
                package_runner.sys,
                "argv",
                ["package-navigation-churn", "--dist-dir", str(self.dist_dir)],
            ),
    ...
    def test_package_mode_normalizes_shared_find_browser_m0_error(self) -> None:
        ...
        package_snapshot = package_runner.capture_package_navigation_churn_snapshot(
            self.dist_dir
        )
        alias_identity = {
        ...
        self.assertIs(package_runner.navigation_churn.M0Error, package_browser_smoke.M0Error)
        stderr = io.StringIO()
        stdout = io.StringIO()
        with (
            mock.patch.object(
            ...
            mock.patch.object(
                package_runner,
                "create_package_navigation_churn_server",
                return_value=server,
            ),
            mock.patch.object(
                package_runner.navigation_churn,
                "artifact_identity",
            ...
            mock.patch.object(package_runner.navigation_churn, "verify_required_exports"),
            mock.patch.object(
                package_runner,
                "find_browser",
    ...
    def test_shared_server_factory_rejects_nonprivate_or_empty_captured_artifacts(self) -> None:
        with self.assertRaisesRegex(M0Error, "artifact names"):
            navigation_churn.create_server_from_artifacts(
                "127.0.0.1",
                0,
