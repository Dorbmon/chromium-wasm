...
class M7PersistentDefaultPartitionShutdownProbeDomSmokeTest(unittest.TestCase):
    ...
    def test_rejects_lease_release_wrong_order_zero_exit_and_broader_claims(self) -> None:
        ...
        missing_renderer_config_reuse_marker = passing_result()
        run = missing_renderer_config_reuse_marker["run"]
        assert isinstance(run, dict)
        markers = run["markers"]
        assert isinstance(markers, list)
        markers.pop(3)
        ...
        missing_indexed_db_marker = passing_result()
        run = missing_indexed_db_marker["run"]
        assert isinstance(run, dict)
        markers = run["markers"]
        assert isinstance(markers, list)
        markers.pop(4)
        ...
        missing_indexed_db_context_marker = passing_result()
        run = missing_indexed_db_context_marker["run"]
        assert isinstance(run, dict)
        markers = run["markers"]
        assert isinstance(markers, list)
        markers.pop(7)
