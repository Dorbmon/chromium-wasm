...
"""Focused contracts for the persistent-default-partition policy probe."""
...
import run_m7_chrome_persistent_default_partition_policy_probe_dom_smoke as smoke
    ...
    *,
    ...
    return {
        ...
        "run": {
            ...
            "normalProcessExitAndAckReceived": True,
            "onExitCount": 1,
        ...
        "quiescence": {
            "callbacksAfterQuietWindow": 7,
            "callbacksAtLifecycleComplete": 7,
            "quiet": True,
...
def request(
    server: smoke.PersistentDefaultPartitionPolicyProbeServer,
    method: str,
    path: str,
...
class M7PersistentDefaultPartitionPolicyProbeDomSmokeTest(unittest.TestCase):
    def test_uses_only_the_dedicated_policy_artifact_and_empty_switch(self) -> None:
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            (
                smoke.M7_POLICY_MARKER_PREFIX
                + "DEFAULT_CONFIG_DEFAULT_NOT_IN_MEMORY",
                smoke.M7_POLICY_MARKER_PREFIX + "FENCE_OK",
                smoke.M7_POLICY_MARKER_PREFIX + "POLICY_PROBE_COMPLETE",
            ),
        )

    def test_accepts_only_the_fixed_policy_configuration_receipt(self) -> None:
        result = passing_result()
        validate(result)

    def test_rejects_wrong_switch_marker_order_fail_and_broader_claims(self) -> None:
        wrong_switch = passing_result()
        run = wrong_switch["run"]
        assert isinstance(run, dict)
        run["arguments"] = ["--wasm-persistent-default-partition-policy-probe=x"]
        ...
        fail_marker = passing_result()
        run = fail_marker["run"]
        assert isinstance(run, dict)
        run["markers"] = [
            smoke.EXPECTED_MARKERS[0],
            smoke.M7_POLICY_FAIL_PREFIX + "configuration",
            smoke.EXPECTED_MARKERS[2],
        ]
        run["noFailMarkerObserved"] = False
        with self.assertRaises(M0Error):
            validate(fail_marker)

        false_claim = passing_result()
        false_claim["profilePersistenceProven"] = True
        with self.assertRaises(M0Error):
            validate(false_claim)
        false_claim = passing_result()
        false_claim["actualStoragePartitionProven"] = True
        with self.assertRaises(M0Error):
            validate(false_claim)
        false_claim = passing_result()
        false_claim["freshDocumentReloadProven"] = True
        with self.assertRaises(M0Error):
            validate(false_claim)
        false_claim = passing_result()
        false_claim["crashRecoveryProven"] = True
        with self.assertRaises(M0Error):
            validate(false_claim)
    ...
    def test_requires_exact_fresh_output_source_selection(self) -> None:
        ...
        with self.assertRaises(M0Error):
            smoke.validate_m7_output_configuration(
                selected,
                Path("out/wasm-chrome-m7-profile-database"),
            )
        with self.assertRaises(M0Error):
            smoke.validate_m7_output_configuration(
                selected
        ...
        with self.assertRaises(M0Error):
            smoke.validate_m7_output_configuration(
                selected
                + (
                    b"enable_chromium_wasm_m7_persistent_default_partition_policy_probe"
                    b" = true\n"
                ),
                smoke.DEFAULT_OUT_DIR,
            )
        with self.assertRaises(M0Error):
            smoke.validate_m7_output_configuration(
                b"enable_chromium_wasm_m7_persistent_default_partition_policy_probe = false\n",
                smoke.DEFAULT_OUT_DIR,
            )
