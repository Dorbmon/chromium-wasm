...
"""Contracts for the bounded M8 typed Wasm-throw page smoke."""
...
# The fixed module imports one Tag(i32), exports thrower(), and its complete
# function body is i32.const 42; throw tag[0]; end. Keep it separate from the
# zero-payload Wasm-throw-to-JavaScript witness.
WASM_THROW_PAYLOAD_MODULE_BYTES = bytes(
    (
        0,
        97,
        115,
...
WASM_THROW_PAYLOAD_MODULE_SHA256 = (
    "19c537f3b269211d8b12d73ce8691529b79a98f802dc7cbc4ae7d1aa7f009a6d"
)


def successful_wasm_throw_payload_result() -> dict[str, object]:
    readiness = {
        "shellReady": True,
        "surfaceReady": True,
    ...
    return {
        "protocol": 1,
        "case": smoke.PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_CASE,
        "scope": smoke.PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_SCOPE,
        "status": "pass",
        ...
        "pageWebAssemblyExceptionImportedI32TagWasmThrowJsCatchPayloadObserved": True,
...
class M8PageWebAssemblyWasmThrowPayloadDomSmokeTest(unittest.TestCase):
    def test_fixed_module_has_exact_bytes_hash_and_throw_opcode(self) -> None:
        self.assertEqual(len(WASM_THROW_PAYLOAD_MODULE_BYTES), 59)
        self.assertEqual(
            hashlib.sha256(WASM_THROW_PAYLOAD_MODULE_BYTES).hexdigest(),
            WASM_THROW_PAYLOAD_MODULE_SHA256,
        )
        self.assertEqual(WASM_THROW_PAYLOAD_MODULE_BYTES[:8], b"\0asm\x01\0\0\0")
        self.assertIn(
            b"\x02\x0c\x01\x03env\x03tag\x04\x00\x00",
            WASM_THROW_PAYLOAD_MODULE_BYTES,
        )
        self.assertIn(
            b"\x0a\x08\x01\x06\x00\x41\x2a\x08\x00\x0b",
            WASM_THROW_PAYLOAD_MODULE_BYTES,
        )

    def test_mode_is_one_fixed_configuration(self) -> None:
        config = smoke.PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_SMOKE_CONFIG
        self.assertEqual(config.mode_id, "page-webassembly-wasm-throw-payload")
        self.assertEqual(
            config.sentinel,
            "CHROMIUM_WASM_M8_PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_DOM",
        )
        self.assertEqual(
            config.case, "browser_page_webassembly_wasm_throw_payload_m8"
        )
        self.assertEqual(
        ...
        self.assertIs(
            smoke.smoke_config_for_page_webassembly_wasm_throw_payload(True),
            config,
        )
        self.assertIs(
            smoke.smoke_config_for_page_webassembly_wasm_throw_payload(False),
            smoke.DEFAULT_SMOKE_CONFIG,
        )

    def test_result_requires_only_the_fixed_typed_throw_witness(self) -> None:
        config = smoke.PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_SMOKE_CONFIG
        smoke.validate_result(
            successful_wasm_throw_payload_result(),
            expected_versions=VERSIONS,
            smoke_config=config,
        )
        for field, expected in config.page_webassembly_expectations:
            with self.subTest(field=field):
                result = copy.deepcopy(successful_wasm_throw_payload_result())
        ...
        result = successful_wasm_throw_payload_result()
    ...
    def test_result_rejects_wrong_or_repeated_typed_throw_marker(self) -> None:
        config = smoke.PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_SMOKE_CONFIG
        result = successful_wasm_throw_payload_result()
        result["stderr"] = [
            smoke.NETWORK_ENABLE_MARKER,
        ...
        result = successful_wasm_throw_payload_result()
        result["stderr"] = successful_wasm_throw_payload_result()[
            "stderr"
        ] + [smoke.PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_MARKER]
    ...
    def test_url_and_server_bind_the_closed_typed_throw_mode_to_its_token(
        self,
    ) -> None:
        config = smoke.PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_SMOKE_CONFIG
        ...
        payload = json.dumps(successful_wasm_throw_payload_result()).encode(
            "utf-8"
        )
        self.assertEqual(
        ...
        result_server.result_token = "fixed-wasm-throw-payload-token"
        ...
        try:
            ...
            binding_url = (
                ...
                "fixed-wasm-throw-payload-token"
            )
            with urlopen(binding_url, timeout=5) as response:
                self.assertEqual(response.status, 200)
                self.assertEqual(
                    json.loads(response.read()),
                    {"protocol": 1, "mode": "page-webassembly-wasm-throw-payload"},
                )
    ...
    def test_host_keeps_module_execution_native_and_mode_token_bound(self) -> None:
        ...
        for expected in (
            ...
            '"page-webassembly-wasm-throw-payload";',
            "PAGE_WEBASSEMBLY_WASM_THROW_PAYLOAD_SMOKE_MODE",
    ...
    def test_native_route_has_one_literal_typed_wasm_throw_witness(self) -> None:
        ...
        self.assertIn("kWasmThrowImportedI32TagJsCatchPayload", protocol_header)
        self.assertIn("--page-webassembly-wasm-throw-payload", runner)
        ...
        for expected in (
            ...
            "error.is(tag)",
            "error.getArg(tag,0)===42",
            "wasm-throw-imported-i32-tag-js-catch-payload-42",
            "chromium-wasm-m8-page-webassembly-wasm-throw-imported-i32-tag-js-catch-payload-42",
            "0,97,115,109,1,0,0,0,1,8,2,96,1,127,0,96,0,0,2,12,1,3,101,110,118,3,116,97,103,4,0,0,3,2,1,1,7,11,1,7,116,104,114,111,119,101,114,0,0,10,8,1,6,0,65,42,8,0,11",
        ):
            with self.subTest(expected=expected):
    ...
    def test_existing_exception_modes_remain_distinct(self) -> None:
        self.assertIs(
            smoke.smoke_config_for_page_webassembly_wasm_throw(True),
