...
"""Node contracts for the M7 Cookie + History + renderer LocalStorage host."""
...
HOST_PATH = (
    TOOLS_DIR / "host" / "chrome_wasm_profile_cookie_history_local_storage_outer_reload_smoke.js"
)
...
export default async function(options) {
  ...
  if (!required.every((argument) => options.arguments.includes(argument))) {
    throw new Error("aggregate Preferences/Cookie/History capability is missing");
  }
  if (options.arguments.some((argument) =>
      argument.includes("bookmark"))) {
    throw new Error("aggregate enabled an unrelated profile service");
  }
...
'''
    return (
        "import {runChromeWasmProfileCookieHistoryLocalStorageOuterReloadFromQuery} from "
        + json.dumps(HOST_URI)
        + ";\nconst loaderSource = "
...
globalThis.document = {
  ...
  querySelector(selector) {
    return new Map([
      ["#m7-profile-cookie-history-local-storage-outer-reload-root", root],
      ["#m7-profile-cookie-history-local-storage-outer-reload-canvas", canvas],
      ["#m7-profile-cookie-history-local-storage-outer-reload-status", status],
      ["#m7-profile-cookie-history-local-storage-outer-reload-versions", versionsElement],
    ]).get(selector) ?? null;
  },
};
...
const artifact = {
  ...
  module_name: "chrome_wasm_m7_profile_cookie_history_local_storage_test",
...
globalThis.location = new URL(
    "https://m7.test/__m7_chrome_profile_cookie_history_local_storage_outer_reload__/?" + query);
...
const bootstrap = {
  ...
  scope:
      "same-origin-two-outer-documents-chrome-wasm-m7-preferences-cookie-" +
      "manager-history-service-renderer-local-storage-one-shared-drain-per-" +
      "module-orderly-reload-only",
  ordinal,
...
const completed = await runChromeWasmProfileCookieHistoryLocalStorageOuterReloadFromQuery();
if (scenario === "pass") {
...
class M7ProfileCookieHistoryLocalStorageOuterReloadHostTest(unittest.TestCase):
    def run_fake_host(
    ...
    def test_host_neither_uses_outer_storage_nor_self_navigates(self) -> None:
        ...
        self.assertNotIn("BOOKMARK_", source)
