...
"""Node contracts for the M7 Cookie + renderer LocalStorage host."""
...
HOST_PATH = (
    TOOLS_DIR / "host" / "chrome_wasm_profile_cookie_local_storage_outer_reload_smoke.js"
)
...
export default async function(options) {
  ...
  if (!required.every((argument) => options.arguments.includes(argument))) {
    throw new Error("aggregate Preferences/Cookie capability is missing");
  }
  if (options.arguments.some((argument) =>
      argument.includes("bookmark") || argument.includes("history"))) {
...
'''
    return (
        "import {runChromeWasmProfileCookieLocalStorageOuterReloadFromQuery} from "
        + json.dumps(HOST_URI)
        + ";\nconst loaderSource = "
...
globalThis.document = {
  ...
  querySelector(selector) {
    return new Map([
      ["#m7-profile-cookie-local-storage-outer-reload-root", root],
      ["#m7-profile-cookie-local-storage-outer-reload-canvas", canvas],
      ["#m7-profile-cookie-local-storage-outer-reload-status", status],
      ["#m7-profile-cookie-local-storage-outer-reload-versions", versionsElement],
    ]).get(selector) ?? null;
  },
};
...
const artifact = {
  ...
  module_name: "chrome_wasm_m7_profile_cookie_local_storage_test",
...
globalThis.location = new URL(
    "https://m7.test/__m7_chrome_profile_cookie_local_storage_outer_reload__/?" + query);
...
const bootstrap = {
  ...
  scope:
      "same-origin-two-outer-documents-chrome-wasm-m7-preferences-cookie-manager-renderer-local-storage-one-shared-drain-per-module-orderly-reload-only",
  ordinal,
...
const completed = await runChromeWasmProfileCookieLocalStorageOuterReloadFromQuery();
if (scenario === "pass") {
...
class M7ProfileCookieLocalStorageOuterReloadHostTest(unittest.TestCase):
    def run_fake_host(
    ...
    def test_host_neither_uses_outer_storage_nor_self_navigates(self) -> None:
        ...
        self.assertNotIn("HISTORY_", source)
