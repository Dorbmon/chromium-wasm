...
"""Node contracts for the M7 five-store outer-reload host."""
...
HOST_PATH = (
    TOOLS_DIR / "host" / "chrome_wasm_profile_bookmark_cookie_history_database_local_storage_outer_reload_smoke.js"
)
...
export default async function(options) {
  ...
  const databaseA = argumentValue("--wasm-profile-database-token-a=");
  const databaseB = argumentValue("--wasm-profile-database-token-b=");
  ...
  const expectedDatabaseMode = globalThis.__ordinal === 1 ?
  ...
  if (!options.arguments.includes(expectedPreferenceMode) ||
      ...
      !options.arguments.includes(expectedDatabaseMode) ||
      localStorage === null ||
      (globalThis.__ordinal === 1 &&
       (preferenceA === null || preferenceB !== null ||
        databaseA === null || databaseB !== null)) ||
      (globalThis.__ordinal === 2 &&
       (preferenceA === null || preferenceB === null ||
        databaseA === null || databaseB === null)) ||
  ...
  if (globalThis.__scenario === "leak") {
    // A 64-character token split 63/1 proves the host retains exactly the
    // maximum proper prefix needed to detect a token across callbacks.
    options.printErr(localStorage.slice(0, -1));
    options.printErr(localStorage.slice(-1));
    return {};
  }
  const rejectedOutput = new Map([
    ["database-phase", "CHROMIUM_WASM_M7_DATABASE_PHASE:task-post"],
    ["abort-pc", "CHROMIUM_WASM_M7_ABORT_PC:fixed-test-checkpoint"],
    ["database-fail", "CHROMIUM_WASM_M7_DATABASE:FAIL stage=database"],
  ]).get(globalThis.__scenario);
  if (rejectedOutput !== undefined) {
    const module = {};
    options.onRuntimeInitialized.call(module);
    options.printErr(rejectedOutput);
    return module;
  }
  ...
  const databaseADigest = databaseA === null ? null : await digest(databaseA);
  const databaseBDigest = databaseB === null ? null : await digest(databaseB);
  const localStorageDigest = await digest(localStorage);
  const prefs = "CHROMIUM_WASM_M7_PREFS:";
  const database = "CHROMIUM_WASM_M7_DATABASE:";
  ...
  options.printErr(database + "READY");
  if (globalThis.__ordinal === 1) {
    options.printErr(
        database + "SQLITE_WRITE_ACCEPTED sha256=" + databaseADigest);
    options.printErr(
        database + "LEVELDB_WRITE_ACCEPTED sha256=" + databaseADigest);
    options.printErr(database + "DATABASES_CLOSED sha256=" + databaseADigest);
  } else if (globalThis.__ordinal === 2) {
    options.printErr(database + "SQLITE_READ_A_OK sha256=" + databaseADigest);
    options.printErr(database + "LEVELDB_READ_A_OK sha256=" + databaseADigest);
    options.printErr(
        database + "SQLITE_WRITE_ACCEPTED sha256=" + databaseBDigest);
    options.printErr(
        database + "LEVELDB_WRITE_ACCEPTED sha256=" + databaseBDigest);
    options.printErr(database + "DATABASES_CLOSED sha256=" + databaseBDigest);
  } else {
    options.printErr(database + "SQLITE_READ_B_OK sha256=" + databaseBDigest);
    options.printErr(database + "LEVELDB_READ_B_OK sha256=" + databaseBDigest);
    options.printErr(database + "DATABASES_CLOSED sha256=" + databaseBDigest);
  }
  options.printErr(prefs + "FENCE_OK sha256=" +
      (globalThis.__ordinal === 1 ? preferenceADigest : preferenceBDigest));
  options.printErr(database + "FENCE_OK sha256=" +
      (globalThis.__ordinal === 1 ? databaseADigest : databaseBDigest));
  options.printErr(storage + "FENCE_OK sha256=" + localStorageDigest);
  options.printErr(prefs + "LEASE_RELEASED");
  options.printErr(database + "LEASE_RELEASED");
  options.printErr(storage + "LEASE_RELEASED");
  return module;
}
'''
    return (
        "import {runChromeWasmProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadFromQuery} from "
        + json.dumps(HOST_URI)
        + ";\nconst loaderSource = "
...
if ((ordinal !== 1 && ordinal !== 2 && ordinal !== 3) ||
    !["pass", "leak", "database-phase", "abort-pc", "database-fail"]
        .includes(scenario) ||
...
globalThis.document = {
  ...
  querySelector(selector) {
    return new Map([
      ["#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-root", root],
      ["#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-canvas", canvas],
      ["#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-status", status],
      ["#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-versions", versionsElement],
    ]).get(selector) ?? null;
  },
};
...
const rawDatabaseA = "7".repeat(64);
const rawDatabaseB = "8".repeat(64);
...
const artifact = {
  ...
  module_name: "chrome_wasm_m7_profile_bookmark_cookie_history_database_local_storage_test",
...
globalThis.location = new URL(
    "https://m7.test/__m7_chrome_profile_bookmark_cookie_history_database_local_storage_outer_reload__/?" + query);
if (location.href.includes(rawA) || location.href.includes(rawB) ||
    location.href.includes(rawDatabaseA) ||
    location.href.includes(rawDatabaseB) ||
...
const bootstrap = {
  ...
  scope:
      ...
      "model-cookie-manager-history-service-renderer-local-storage-sqlite-" +
      "leveldb-one-" +
  ...
  databaseTokenA: ordinal === 3 ? null : rawDatabaseA,
  databaseTokenB: ordinal === 1 ? null : rawDatabaseB,
  ...
  databaseTokenADigest:
      ordinal === 3 ? null :
          await digest(new TextEncoder().encode(rawDatabaseA)),
  databaseTokenBDigest:
      ordinal === 1 ? null :
          await digest(new TextEncoder().encode(rawDatabaseB)),
};
...
let rejected = null;
try {
  completed = await runChromeWasmProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadFromQuery();
} catch (error) {
  rejected = error;
}
if (scenario === "pass") {
  ...
  const expectedFatal = scenario === "database-fail" ?
      "native-fixed-failure" : "database-diagnostic-telemetry";
  if (rejected === null || completed !== null || result === null || ready !== null ||
      result.status !== "fail" || !result.fatalErrors.includes(expectedFatal) ||
      result.run === null || result.run.markers.length !== 0 ||
      !result.run.stderr.every((line) => line === "<suppressed-native-output>")) {
    throw new Error("native output rejection evidence is invalid");
  }
...
class M7ProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadHostTest(unittest.TestCase):
    def run_fake_host(
    ...
    def test_rejects_63_plus_1_split_token_leak_without_echo(self) -> None:
    ...
    def test_rejects_database_phase_telemetry_in_capture_output(self) -> None:
        completed = self.run_fake_host(1, "database-phase")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertEqual(completed.stdout, "rejected\n")

    def test_rejects_abort_pc_telemetry_in_capture_output(self) -> None:
        completed = self.run_fake_host(1, "abort-pc")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertEqual(completed.stdout, "rejected\n")

    def test_rejects_database_failure_marker_in_capture_output(self) -> None:
        completed = self.run_fake_host(1, "database-fail")
        self.assertEqual(completed.returncode, 0, completed.stderr)
        self.assertEqual(completed.stdout, "rejected\n")
    ...
    def test_host_neither_uses_outer_storage_nor_self_navigates(self) -> None:
        ...
        self.assertIn("--wasm-profile-database-smoke=write-a", source)
        self.assertIn("--wasm-profile-database-smoke=verify-a-write-b", source)
        self.assertIn("--wasm-profile-database-smoke=verify-b", source)
        self.assertIn("--wasm-profile-database-token-a=", source)
        self.assertIn("--wasm-profile-database-token-b=", source)
        ...
        self.assertIn("SQLITE_WRITE_ACCEPTED", source)
        self.assertIn("LEVELDB_WRITE_ACCEPTED", source)
        self.assertIn("SQLITE_READ_A_OK", source)
        self.assertIn("LEVELDB_READ_A_OK", source)
        self.assertIn("SQLITE_READ_B_OK", source)
        self.assertIn("LEVELDB_READ_B_OK", source)
        self.assertIn("DATABASES_CLOSED", source)
        self.assertIn("databaseLeaseReleasedMarkerObserved", source)
        self.assertIn("sharedDrainReceiptsAccepted", source)
        self.assertIn(
            "combined.slice(-(TOKEN_BYTES * 2 - 1))", source
        )
