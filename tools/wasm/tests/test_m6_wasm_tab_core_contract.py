...
class M6WasmTabCoreContractTest(unittest.TestCase):
    ...
    def test_target_is_narrow_and_reached_only_by_the_smoke_owner(self) -> None:
        ...
        # The two shutdown lifecycles, source-selected Browser/runtime smoke,
        # and the bounded tab-strip observer create/use the model directly.
        # These are nine explicit Wasm-only consumers, not desktop UI routes.
        self.assertEqual(9, wasm_build.count('":wasm_tab_core",'))
        self.assertIn(
            '":wasm_tab_core",',
        ...
        self.assertIn(
            '":wasm_tab_core",',
            _source_set_body(wasm_build, "wasm_browser_lifecycle"),
        )
        self.assertIn(
            '":wasm_tab_core",',
        ...
        self.assertIn(
            '":wasm_tab_core",',
            _source_set_body(wasm_build, "wasm_browser_smoke"),
        )
        self.assertNotIn(
            '":wasm_tab_core",',
