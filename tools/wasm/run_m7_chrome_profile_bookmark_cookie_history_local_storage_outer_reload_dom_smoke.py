...
"""Run the aggregate Bookmark + Cookie + History + renderer LocalStorage outer-reload witness.

Document one writes preference A, Bookmark A, Cookie A, and History A, then
writes one separately escrowed token through a real renderer's
``window.localStorage``. Only after the Browser, BookmarkModel, Cookie SQLite
backend, History/Favicons SQLite backends, renderer LocalStorage database,
Preferences fence, and the one shared V4 drain have completed may both
protocol-specific ``LEASE_RELEASED`` receipts appear. The runner then issues
...
and History B, and reopens the same LocalStorage token through Blink. After the
same ordered close/fence/drain boundary, a second ``Page.reload`` creates
document three. It reopens preference B, Bookmark B, Cookie B, both History
rows, and renderer LocalStorage; removes Bookmark B with a result-bearing
cleanup flush; and reaches the same shared drain again.
...
general Cookie request path, or a generally persistent StoragePartition. Raw
preference A, preference B, and LocalStorage values exist only in this
...
SENTINEL = "CHROMIUM_WASM_M7_CHROME_PROFILE_BOOKMARK_COOKIE_HISTORY_LOCAL_STORAGE_OUTER_RELOAD_DOM"
...
PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_bookmark_cookie_history_local_storage_test"
...
DEFAULT_OUT_DIR = Path("out/wasm-chrome-m7-profile-bookmark-cookie-history-local-storage")
...
HOST_ROOT = "/__m7_chrome_profile_bookmark_cookie_history_local_storage_outer_reload__"
HOST_HTML_NAME = "chrome_wasm_profile_bookmark_cookie_history_local_storage_outer_reload_smoke.html"
HOST_JS_NAME = "chrome_wasm_profile_bookmark_cookie_history_local_storage_outer_reload_smoke.js"
...
M7_GN_ENABLE_ASSIGNMENT_RE = re.compile(
    r"^[ \t]*enable_chromium_wasm_m7_profile_bookmark_cookie_history_local_storage_test[ \t]*="
    r"[ \t]*(?:\r?\n[ \t]+)?(true|false)[ \t]*(?:#.*)?$",
    re.MULTILINE,
)
...
class ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadServer(ThreadingHTTPServer):
    daemon_threads = True
...
class ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadRequestHandler(BaseHTTPRequestHandler):
    server: ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadServer
...
def create_server(
    ...
    *,
...
) -> ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadServer:
    ...
    artifacts = snapshot_regular_files(
        ...
        description="outer-reload profile Bookmark/Cookie/History/LocalStorage artifacts",
    )
    ...
    server = ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadServer(
        (host, port), ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadRequestHandler
    )
...
def verify_server_delivery(server: ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadServer) -> None:
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = (
        diagnostics_dir
        / "chrome-profile-bookmark-cookie-history-local-storage-outer-reload-m7-failure.json"
    )
    payload = {
        "schema_version": 1,
        "runner": "run_m7_chrome_profile_bookmark_cookie_history_local_storage_outer_reload_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "stage": stage,
        "nonclaims": [
...
def _stop_server(
    server: ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadServer | None,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run three aggregate Bookmark + Cookie + History + renderer LocalStorage Chrome Wasm "
            "documents separated by two real DevTools Page.reload calls."
        ),
        epilog=(
            "Build the normal artifact with: buildtools/linux64/gn gen "
            "out/wasm-chrome-m7-profile-bookmark-cookie-history-local-storage --args='"
            + DEFAULT_GN_ARGUMENTS
            + "' --fail-on-unused-args; autoninja -C "
            "out/wasm-chrome-m7-profile-bookmark-cookie-history-local-storage chrome_wasm"
        ),
    )
    ...
    server: ChromeProfileBookmarkCookieHistoryLocalStorageOuterReloadServer | None = None
    ...
    try:
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-profile-bookmark-cookie-history-local-storage-outer-reload-server",
            daemon=True,
        )
        server_thread.start()
        ...
        outer_profile = tempfile.TemporaryDirectory(
            prefix="chromium-wasm-m7-profile-bookmark-cookie-history-local-storage-outer-reload-"
        )
        ...
        stderr_thread = threading.Thread(
            ...
            name=(
                "chromium-wasm-m7-profile-bookmark-cookie-history-local-storage-"
