...
"""Run trusted DOM Menu -> Settings input through Chrome's Wasm host.

Only physical DevTools mouse events are sent to C++-reported canvas targets.
The shared host pointer adapter forwards those records through Ozone/Aura; a
switch-gated C++ verifier observes the real BrowserView menu, WebUI navigation,
and post-action compositor frames. The resulting Settings page is explicitly
the limited, read-only M6 bootstrap, not desktop Chrome Settings.
"""
...
SENTINEL = "CHROMIUM_WASM_M6_HOST_POINTER_MENU_DOM"
CASE = "browser_host_pointer_menu_settings_m6"
SCOPE = "trusted-dom-pointer-ozone-aura-views-menu-settings-bootstrap"
SWITCH = "--wasm-browser-host-pointer-menu-smoke"
READY_MARKER = "CHROMIUM_WASM_M6_HOST_POINTER_MENU:READY"
MENU_OPEN_MARKER = "CHROMIUM_WASM_M6_HOST_POINTER_MENU:MENU_OPEN"
MENU_PRESENTED_MARKER = "CHROMIUM_WASM_M6_HOST_POINTER_MENU:MENU_PRESENTED"
MENU_CLOSED_MARKER = "CHROMIUM_WASM_M6_HOST_POINTER_MENU:MENU_CLOSED"
SETTINGS_NAVIGATED_MARKER = "CHROMIUM_WASM_M6_HOST_POINTER_MENU:SETTINGS_NAVIGATED"
PASS_MARKER = "CHROMIUM_WASM_M6_HOST_POINTER_MENU:PASS"
LIMITED_SETTINGS_BOOTSTRAP = "limited-m6-bootstrap-read-only-volatile"
MODULE_NAME_RE = re.compile(r"^[A-Za-z0-9_]+$")
MAX_RESULT_BYTES = 1024 * 1024
MAX_FRAME_DIMENSION = 16384
HOST_ROOT = "/__m6_browser_host_pointer_menu__"


class HostPointerMenuSmokeServer(ThreadingHTTPServer):
    daemon_threads = True
...
class HostPointerMenuSmokeRequestHandler(BaseHTTPRequestHandler):
    server: HostPointerMenuSmokeServer
    ...
    def do_GET(self) -> None:
        ...
        if path in (HOST_ROOT, f"{HOST_ROOT}/"):
            ...
            return
        if path == f"{HOST_ROOT}/chrome_wasm_browser_host_pointer_menu_smoke_host.js":
            self._send_bytes(
                HTTPStatus.OK,
    ...
    def do_POST(self) -> None:
        ...
        if result is None:
            self._send_bytes(
                ...
                "text/plain; charset=utf-8",
                b"invalid host-pointer-menu result\n",
            )
            return
        with self.server.result_lock:
            if self.server.result_received:
                self._send_bytes(
                    HTTPStatus.CONFLICT,
                    "text/plain; charset=utf-8",
                    b"host-pointer-menu result already received\n",
                )
                return
            try:
                ...
                self._send_bytes(
                    HTTPStatus.CONFLICT,
                    "text/plain; charset=utf-8",
                    b"host-pointer-menu result queue is full\n",
                )
                return
...
def create_server(
    ...
    *,
    ...
    server.html_bytes = (
        host_dir / "chrome_wasm_browser_host_pointer_menu_smoke.html"
    ).read_bytes()
    server.host_js_bytes = (
        host_dir / "chrome_wasm_browser_host_pointer_menu_smoke_host.js"
    ).read_bytes()
...
def _require_equal(result: dict[str, Any], field: str, expected: object) -> None:
    if not browser_view_smoke._exact_json_value_equal(result.get(field), expected):
        raise M0Error(
            f"host-pointer-menu result {field} mismatch: expected {expected!r}, "
...
def _validate_target(value: object, name: str) -> dict[str, object]:
    if not isinstance(value, dict):
        raise M0Error(f"host-pointer-menu {name} target is missing")
    for field in ("x", "y"):
        coordinate = value.get(field)
        if type(coordinate) is not int or not 0 <= coordinate < MAX_FRAME_DIMENSION:
            raise M0Error(f"host-pointer-menu {name} target {field} is invalid")
...
def _validate_frame_order(value: dict[str, Any], before_field: str, after_field: str) -> None:
    ...
    if type(before) is not int or type(after) is not int or before < 0 or after <= before:
        raise M0Error(
            "host-pointer-menu has no ordered presentation evidence for "
            f"{after_field}"
        )
...
def _validate_pointer_action(
    ...
    if not isinstance(record, dict):
        raise M0Error(f"host-pointer-menu action {index} is not an object")
    expected = {
        "type": event_type,
        "trusted": True,
        "cancelable": True,
    ...
    for field, expected_value in expected.items():
        if record.get(field) != expected_value:
            raise M0Error(
                f"host-pointer-menu action {index} {field} is invalid: "
...
def _validate_host_input(value: object) -> None:
    if not isinstance(value, dict):
        raise M0Error("host-pointer-menu result has no host-input evidence")
    for field in (
        "attached",
        "readyObserved",
        "menuOpenedObserved",
        ...
        if value.get(field) is not True:
            raise M0Error(f"host-pointer-menu input {field} is not true")
    if value.get("settingsBootstrap") != LIMITED_SETTINGS_BOOTSTRAP:
        raise M0Error("host-pointer-menu result does not describe limited Settings")
    menu_target = _validate_target(value.get("menuTarget"), "Menu")
    settings_target = _validate_target(value.get("settingsTarget"), "Settings")
    if (menu_target["x"], menu_target["y"]) == (
        settings_target["x"],
        settings_target["y"],
    ):
        raise M0Error("host-pointer-menu Menu and Settings targets must differ")
    _validate_frame_order(
        value, "frameIdAtMenuOpenedMarker", "frameIdAfterMenuOpen"
    )
    _validate_frame_order(
        value, "frameIdAtMenuClosedMarker", "frameIdAfterSettingsClick"
    )
    _validate_frame_order(
        value,
        "frameIdAtSettingsNavigatedMarker",
        "frameIdAfterSettingsNavigation",
    )
    if value["frameIdAtMenuClosedMarker"] < value["frameIdAfterMenuOpen"]:
        raise M0Error(
            "host-pointer-menu Settings click did not wait for Menu presentation"
        )

    records = value.get("pointerRecords")
    if not isinstance(records, list):
        raise M0Error("host-pointer-menu pointer records are missing")
    rejected = [
        record
    ...
    if rejected:
        raise M0Error("host-pointer-menu rejected an outer trusted pointer record")
    actions = [
        record
        for record in records
    ...
    if len(actions) != 4:
        raise M0Error("host-pointer-menu has not recorded exactly two pointer clicks")
    _validate_pointer_action(actions[0], menu_target, "down", 1, 0)
    _validate_pointer_action(actions[1], menu_target, "up", 0, 1)
    _validate_pointer_action(actions[2], settings_target, "down", 1, 2)
    _validate_pointer_action(actions[3], settings_target, "up", 0, 3)
...
def validate_result(result: dict[str, Any], *, expected_versions: dict[str, str]) -> None:
    ...
    if process_exit_code is not None and process_exit_code != 0:
        raise M0Error("host-pointer-menu process exit disagrees with runtime")
    if result.get("versions") != expected_versions:
        raise M0Error("host-pointer-menu versions do not match manifest")
    for field in ("fatalErrors", "windowErrors", "unhandledRejections"):
        if not isinstance(result.get(field), list) or result[field]:
            raise M0Error(f"host-pointer-menu {field} is not empty")
    for field in ("stdout", "stderr"):
        if not isinstance(result.get(field), list):
            raise M0Error(f"host-pointer-menu {field} is not a list")
    ...
    for marker in (
        ...
        if marker not in stderr:
            raise M0Error(f"host-pointer-menu stderr is missing {marker}")
...
def _take_early_result(
    ...
    try:
        ...
        return
    raise M0Error(
        f"host-pointer-menu smoke finished before {stage}: "
...
def wait_for_state(
    ...
    # This is a fixed read-only state expression. It cannot focus, dispatch,
    # mutate the proxy, invoke an export, or navigate the Wasm application.
    expression = "globalThis.__chromiumWasmM6HostPointerMenuState || null"
    while time.monotonic() < deadline:
        ...
        time.sleep(0.05)
    raise M0Error(
        f"host-pointer-menu smoke did not reach {desired}: "
...
def click_target(client: Any, state: dict[str, object], field: str) -> None:
    ...
    if not isinstance(target, dict):
        raise M0Error(f"host-pointer-menu state lacks {field}")
    x = target.get("clientX")
    y = target.get("clientY")
    if not isinstance(x, (int, float)) or not isinstance(y, (int, float)):
        raise M0Error(f"host-pointer-menu {field} client coordinates are invalid")
    # This is Chrome DevTools physical mouse input. The runner does not call
    # a Wasm export, manipulate the BrowserView, or evaluate a page command.
...
def wait_for_result(
    ...
    while True:
        if browser.poll() is not None:
            raise M0Error(
                "host browser exited before host-pointer-menu result: "
                + "\n".join(browser_stderr)
            )
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            raise M0Error("host-pointer-menu smoke timeout: " + "\n".join(browser_stderr))
        try:
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = diagnostics_dir / "chrome-browser-host-pointer-menu-m6-failure.json"
    payload = {
        "schema_version": 1,
        "runner": "run_m6_wasm_browser_host_pointer_menu_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "stage": stage,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run trusted DOM Menu -> Settings pointer input through Chrome Wasm."
    )
    ...
    server: HostPointerMenuSmokeServer | None = None
    ...
    try:
        ...
        for suffix in (".js", ".wasm"):
            if not (out_dir / f"{args.module_name}{suffix}").is_file():
                raise M0Error(f"host-pointer-menu artifact is missing: {suffix}")
        stage = "load_manifest"
        manifest = load_manifest()
        versions = manifest_versions(manifest, checked_output(["git", "rev-parse", "HEAD"]))
        context = print_context(
            "run_m6_wasm_browser_host_pointer_menu_dom_smoke.py",
            manifest,
            case=CASE,
            scope=SCOPE,
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m6-host-pointer-menu-server",
            daemon=True,
        )
        server_thread.start()
        url = smoke_url(
        ...
        profile = tempfile.TemporaryDirectory(prefix="chromium-wasm-m6-host-pointer-menu-")
        ...
        stderr_thread = threading.Thread(
            ...
            name="chromium-wasm-m6-host-pointer-menu-browser-stderr",
            daemon=True,
        )
        stderr_thread.start()
        ...
        stage = "wait_for_dynamic_settings_target"
        state = wait_for_state(
            client,
            browser,
            browser_stderr,
        ...
        stage = "dispatch_trusted_dom_settings"
