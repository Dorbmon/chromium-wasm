...
"""Run the trusted DOM committed-text Chrome Wasm smoke in a real browser."""
...
SENTINEL = "CHROMIUM_WASM_M6_HOST_TEXT_DOM"
CASE = "browser_host_text_m6"
SCOPE = "trusted-dom-beforeinput-ozone-textinputclient-navigation"
SWITCH = "--wasm-browser-host-text-smoke"
ADDRESS_TEXT_CHUNKS = ("chrome://", "version/")
ADDRESS_TEXT = "".join(ADDRESS_TEXT_CHUNKS)
BURST_ARMED_MARKER = "CHROMIUM_WASM_M6_HOST_TEXT:BURST_ARMED"
READY_MARKER = "CHROMIUM_WASM_M6_HOST_TEXT:READY"
FOCUSED_MARKER = "CHROMIUM_WASM_M6_HOST_TEXT:FOCUSED"
INSERTED_MARKER = "CHROMIUM_WASM_M6_HOST_TEXT:TEXT_INSERTED"
NAVIGATED_MARKER = "CHROMIUM_WASM_M6_HOST_TEXT:NAVIGATED"
PASS_MARKER = "CHROMIUM_WASM_M6_HOST_TEXT:PASS"
MODULE_NAME_RE = re.compile(r"^[A-Za-z0-9_]+$")
MAX_RESULT_BYTES = 1024 * 1024
MAX_FRAME_DIMENSION = 16384
HOST_ROOT = "/__m6_browser_host_text__"


class HostTextSmokeServer(ThreadingHTTPServer):
    daemon_threads = True
...
class HostTextSmokeRequestHandler(BaseHTTPRequestHandler):
    server: HostTextSmokeServer
    ...
    def do_GET(self) -> None:
        ...
        if path in (HOST_ROOT, f"{HOST_ROOT}/"):
            ...
            return
        if path == f"{HOST_ROOT}/chrome_wasm_browser_host_text_smoke_host.js":
            self._send_bytes(
                HTTPStatus.OK,
        ...
        with self.server.result_lock:
            if self.server.result_received:
                self._send_bytes(
                    HTTPStatus.CONFLICT,
                    "text/plain; charset=utf-8",
                    b"host-text result already received\n",
                )
                return
            try:
                ...
                self._send_bytes(
                    HTTPStatus.CONFLICT,
                    "text/plain; charset=utf-8",
                    b"host-text result queue is full\n",
                )
                return
...
def create_server(
    ...
    *,
    ...
    if not out_dir.is_dir():
        raise M0Error(f"host-text output directory is missing: {out_dir}")
    host_dir = Path(__file__).with_name("host")
    server = HostTextSmokeServer((host, port), HostTextSmokeRequestHandler)
    ...
    server.html_bytes = (
        host_dir / "chrome_wasm_browser_host_text_smoke.html"
    ).read_bytes()
    server.host_js_bytes = (
        host_dir / "chrome_wasm_browser_host_text_smoke_host.js"
    ).read_bytes()
...
def _require_equal(result: dict[str, Any], field: str, expected: object) -> None:
    if not browser_view_smoke._exact_json_value_equal(result.get(field), expected):
        raise M0Error(
            f"host-text result {field} mismatch: expected {expected!r}, "
...
def _validate_presentation_pair(value: dict[str, Any], before: str, after: str) -> None:
    ...
    if (
        ...
        raise M0Error(f"host-text lacks fresh presentation evidence for {after}")


def _validate_host_input(value: object) -> None:
    if not isinstance(value, dict):
        raise M0Error("host-text result has no host-input evidence")
    for field in (
        "attached",
        "readyObserved",
        "burstArmedObserved",
        "nativeBurstAdmissionsObserved",
        "focusObserved",
        "focusPresentationObserved",
        "insertedObserved",
        "insertedPresentationObserved",
        "navigatedObserved",
        ...
        if value.get(field) is not True:
            raise M0Error(f"host-text input {field} is not true")
    for field in ("deliveryRejected", "proxySessionCleared"):
        if value.get(field) is not False:
            raise M0Error(f"host-text input {field} is not false")
    if value.get("textareaValue") != "":
        raise M0Error("host-text textarea default was not prevented")
    if value.get("nativeTextAdmissionCount") != len(ADDRESS_TEXT_CHUNKS):
        raise M0Error("host-text did not admit both native text chunks")
    if value.get("nativeTextDeliveryCount") != len(ADDRESS_TEXT_CHUNKS):
        raise M0Error("host-text did not deliver both native text chunks")
    if value.get("nativeTextDeliveryCountAtAdmission") != [0, 0]:
        raise M0Error("host-text observed an acknowledgement before burst admission")
    if value.get("nativeTextDeliverySequences") != [1, 2]:
        raise M0Error("host-text native delivery sequence is not ordered")
    if value.get("pendingDeliveryCount") != 0 or value.get(
        "pendingTextUtf8Bytes"
    ) != 0:
        raise M0Error("host-text retained native text after delivery")
    if value.get("tombstonedDeliveryCount") != 0:
        raise M0Error("host-text unexpectedly tombstoned a live delivery")
    for before, after in (
        ("focusMarkerFrameId", "frameIdAfterFocus"),
        ("insertedMarkerFrameId", "frameIdAfterInsert"),
        ("navigationMarkerFrameId", "frameIdAfterNavigation"),
    ):
        _validate_presentation_pair(value, before, after)
    if value.get("frameIdAfterFocus") > value.get("insertedMarkerFrameId"):
        raise M0Error("host-text insert marker predates focused address presentation")
    if value.get("frameIdAfterInsert") > value.get("navigationMarkerFrameId"):
        raise M0Error("host-text navigation marker predates inserted-text presentation")
    ...
    if not isinstance(ctrl_l, list) or len(ctrl_l) != len(expected_ctrl_l):
        raise M0Error("host-text Ctrl+L DOM record count is invalid")
    for index, (event_type, code) in enumerate(expected_ctrl_l):
        record = ctrl_l[index]
        if not isinstance(record, dict):
            raise M0Error("host-text Ctrl+L record is invalid")
        for field, expected in {
            ...
            if record.get(field) != expected:
                raise M0Error(f"host-text Ctrl+L record {index} {field} is invalid")

    beforeinput = value.get("beforeInputRecords")
    if not isinstance(beforeinput, list) or len(beforeinput) != len(
        ADDRESS_TEXT_CHUNKS
    ):
        raise M0Error("host-text beforeinput record count is invalid")
    for index, chunk in enumerate(ADDRESS_TEXT_CHUNKS):
        beforeinput_record = beforeinput[index]
        if not isinstance(beforeinput_record, dict):
            raise M0Error("host-text beforeinput record is invalid")
        if "data" in beforeinput_record:
            raise M0Error("host-text beforeinput diagnostics retained raw text")
        for field, expected in {
            ...
            if beforeinput_record.get(field) != expected:
                raise M0Error(
                    f"host-text beforeinput {field} at {index} is invalid"
                )

    delivery = value.get("browserTextDeliveryReports")
    if delivery != [
        {"action": 4, "sessionId": 0, "sequence": 1, "accepted": True},
        {"action": 4, "sessionId": 0, "sequence": 2, "accepted": True},
    ]:
        raise M0Error("host-text native delivery does not exactly acknowledge action 4")

    enter = value.get("enterRecords")
    if not isinstance(enter, list) or len(enter) != 2:
        raise M0Error("host-text Enter DOM record count is invalid")
    for index, event_type in enumerate(("keydown", "keyup")):
        record = enter[index]
        if not isinstance(record, dict):
            raise M0Error("host-text Enter record is invalid")
        for field, expected in {
            "type": event_type,
            "code": "Enter",
            "key": "Enter",
            ...
            if record.get(field) != expected:
                raise M0Error(f"host-text Enter record {index} {field} is invalid")

    for field in ("rejectedRecords", "cleanupRecords"):
        if value.get(field) != []:
            raise M0Error(f"host-text has unexpected {field}")
...
def validate_result(result: dict[str, Any], *, expected_versions: dict[str, str]) -> None:
    ...
    if process_exit_code is not None and process_exit_code != 0:
        raise M0Error("host-text bridge process exit disagrees with runtime")
    if result.get("versions") != expected_versions:
        raise M0Error("host-text versions do not match the manifest")
    for field in ("fatalErrors", "windowErrors", "unhandledRejections"):
        if not isinstance(result.get(field), list) or result[field]:
            raise M0Error(f"host-text {field} is not empty")
    for field in ("stdout", "stderr"):
        if not isinstance(result.get(field), list):
            raise M0Error(f"host-text {field} is not a list")
    ...
    for marker in (
        ...
        if marker not in stderr:
            raise M0Error(f"host-text stderr is missing {marker}")
    ...
    if not isinstance(text_states, list) or not any(
        isinstance(state, dict) and state.get("focusedClientPresent") is True
        and state.get("editable") is True
        for state in text_states
    ):
        raise M0Error("host-text has no editable Ozone TextInputClient state")
...
def wait_for_state(
    ...
    expression = "globalThis.__chromiumWasmM6HostTextState || null"
    while time.monotonic() < deadline:
        ...
        if early_result is not None:
            raise M0Error(
                f"host-text smoke finished before {state}: "
        ...
        time.sleep(0.05)
    raise M0Error(
        f"host-text smoke did not become ready for {state}: "
        + json.dumps(last_state, sort_keys=True, separators=(",", ":"))
    )


def dispatch_unmodified_enter(client: Any) -> None:
    # Keep this a physical CDP key sequence. In particular, no Runtime
    # expression, JS navigation API, or browser-text C ABI appears in the
    # runner after committed text has been injected by Input.insertText.
...
def verify_explicit_text_heap_exports(module_loader: Path) -> None:
    try:
        ...
        raise M0Error(f"cannot read host-text module loader: {error}") from error
    for export in ('Module["_malloc"]', 'Module["_free"]'):
        if export not in loader:
            raise M0Error(
                f"host-text module does not expose required explicit heap export {export}"
            )
...
def wait_for_result(
    ...
    while True:
        if browser.poll() is not None:
            raise M0Error(
                "host browser exited before host-text result: "
                + "\n".join(browser_stderr)
            )
        remaining = deadline - time.monotonic()
        if remaining <= 0:
            raise M0Error("host-text smoke timeout: " + "\n".join(browser_stderr))
        try:
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = diagnostics_dir / "chrome-browser-host-text-m6-failure.json"
    payload = {
        "schema_version": 1,
        "runner": "run_m6_wasm_browser_host_text_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "stage": stage,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run trusted DOM committed text through Chrome's Wasm Ozone host."
    )
    ...
    server: HostTextSmokeServer | None = None
    ...
    try:
        ...
        for suffix in (".js", ".wasm"):
            if not (out_dir / f"{args.module_name}{suffix}").is_file():
                raise M0Error(f"host-text artifact is missing: {suffix}")
        ...
        context = print_context(
            "run_m6_wasm_browser_host_text_dom_smoke.py",
            manifest,
            case=CASE,
            scope=SCOPE,
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m6-host-text-server",
            daemon=True,
        )
        server_thread.start()
        url = smoke_url(
        ...
        profile = tempfile.TemporaryDirectory(prefix="chromium-wasm-m6-host-text-")
        ...
        stderr_thread = threading.Thread(
            ...
            name="chromium-wasm-m6-host-text-browser-stderr",
            daemon=True,
        )
        stderr_thread.start()
        ...
        stage = "dispatch_trusted_input_insert_text_burst"
