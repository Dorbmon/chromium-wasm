...
"""Prove a Chromium SQLite same-process lock holder/release sequence.

The isolated SQLite lock artifact uses two Chromium sql::Database handles on
one file in the V4-mounted /profile/Default test-profile directory. The
holder's real rollback-journal write of A keeps the transaction open while the
contender's real B write must return SQLite BUSY. The contender closes before
holder commit; fresh handles then reopen/integrity-check A, write B, and
reopen/integrity-check B. This is not a full-profile concurrent-writer,
cross-process, direct-fcntl, WAL/mmap, or normal-profile persistence test.

The runner keeps both raw tokens exclusively in in-memory escrow. The page
gets them through a one-use same-origin no-store bootstrap response; neither
appears in a URL, result receipt, diagnostic, standard output, or browser
stderr.
"""
...
SENTINEL = "CHROMIUM_WASM_M7_CHROME_PROFILE_DATABASE_SQLITE_LOCK_DOM"
CASE = "chrome_profile_database_sqlite_lock_m7"
SCOPE = (
    "same-origin-same-document-one-chrome-wasm-m7-profile-database-sqlite-lock-test-module"
)
PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_database_sqlite_lock_test"
...
DEFAULT_OUT_DIR = Path("out/wasm-chrome-m7-profile-database-sqlite-lock")
DEFAULT_GN_ARGUMENTS = (
    'import("//out/wasm-chrome-m6/args.gn") '
    "enable_chromium_wasm_m7_profile_database_test=true "
    + PRODUCT_GN_ENABLE_ARGUMENT
)
HOST_ROOT = "/__m7_chrome_profile_database_sqlite_lock__"
HOST_HTML_NAME = "chrome_wasm_profile_database_sqlite_lock_smoke.html"
HOST_JS_NAME = "chrome_wasm_profile_database_sqlite_lock_smoke.js"
...
M7_DATABASE_TEST_SELECTOR = "enable_chromium_wasm_m7_profile_database_test"
M7_SQLITE_LOCK_SELECTOR = (
    "enable_chromium_wasm_m7_profile_database_sqlite_lock_test"
)
M7_INCOMPATIBLE_SELECTORS = (
...
def _require_product_module_name(module_name: object, boundary: str) -> str:
    if not isinstance(module_name, str) or module_name != PRODUCT_MODULE_NAME:
        raise M0Error(f"SQLite lock {boundary} module is invalid")
    return module_name


def validate_m7_output_configuration(args_gn: bytes) -> None:
    """Require only the source-selected one-module SQLite lock acceptance."""

    try:
        text = args_gn.decode("utf-8").replace("\r\n", "\n")
    except UnicodeDecodeError as exc:
        raise M0Error("SQLite lock args.gn is not UTF-8") from exc
    selector_values = {selector: [] for selector in M7_GN_SELECTORS}
    for raw_line in text.splitlines():
        code = raw_line.split("#", 1)[0].strip()
        if not code:
            continue
        for selector in M7_GN_SELECTORS:
            if not code.startswith(selector):
                continue
            suffix = code[len(selector) :]
            if suffix and (suffix[0].isalnum() or suffix[0] == "_"):
                continue
            assignment = re.fullmatch(
                re.escape(selector) + r"[ \t]*=[ \t]*(true|false)", code
            )
            if assignment is None:
                raise M0Error(
                    "SQLite lock args.gn has a nonliteral selector assignment"
                )
            selector_values[selector].append(assignment.group(1))
            break
    if selector_values[M7_DATABASE_TEST_SELECTOR] != ["true"]:
        raise M0Error("SQLite lock args.gn lacks the database test opt-in")
    if selector_values[M7_SQLITE_LOCK_SELECTOR] != ["true"]:
        raise M0Error("SQLite lock args.gn lacks its lock-test opt-in")
    for selector in M7_INCOMPATIBLE_SELECTORS:
        if selector_values[selector] not in ([], ["false"]):
            raise M0Error("SQLite lock args.gn enables an incompatible diagnostic")
...
def create_server(
    ...
    *,
    ...
    artifacts = snapshot_regular_files(
        ...
        description="SQLite lock profile database artifacts",
    )
    args_gn = snapshot_regular_file(
        out_dir / "args.gn",
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="SQLite lock selected args.gn",
    )
    ...
    host_snapshots = snapshot_regular_files(
        ...
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="SQLite lock host resources",
    )
    runner_source = snapshot_regular_file(
        runner_source_path or Path(__file__),
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="SQLite lock runner source",
    )
    server = ChromeProfileDatabaseLockServer(
...
def expected_markers(escrow: TokenEscrow) -> list[str]:
    return [
        ...
        f"{M7_DATABASE_MARKER_PREFIX}SQLITE_LOCK_HOLDER_WRITE_A_ACCEPTED "
        f"sha256={escrow.token_a_digest}",
        f"{M7_DATABASE_MARKER_PREFIX}SQLITE_LOCK_CONTENDER_BUSY",
        f"{M7_DATABASE_MARKER_PREFIX}SQLITE_LOCK_RELEASE_REOPEN_A_INTEGRITY_OK "
        f"sha256={escrow.token_a_digest}",
        f"{M7_DATABASE_MARKER_PREFIX}"
...
def _validate_token_evidence(value: object, escrow: TokenEscrow) -> None:
    ...
    if (
        ...
        or tokens.get("tokenB") != escrow.token_b_digest
...
def _validate_run(value: object, escrow: TokenEscrow) -> None:
    ...
    if (
        run.get("mode") != "sqlite-lock-contention"
...
def validate_result(
    ...
    *,
    ...
    """Accept only the exact SQLite holder/rejection/release receipt."""
    ...
    for field, expected in {
        ...
        "sameProcessLevelDbLockContentionReleaseProven": False,
        "sameProcessSqliteLockContentionReleaseProven": True,
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = diagnostics_dir / "chrome-profile-database-sqlite-lock-m7-failure.json"
    payload = {
        "schema_version": 1,
        "runner": "run_m7_chrome_profile_database_sqlite_lock_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "m7GateComplete": False,
        "stage": stage,
...
def sqlite_lock_summary() -> dict[str, object]:
    ...
    return {
        ...
        "sameProcessLevelDbLockContentionReleaseProven": False,
        "sameProcessSqliteLockContentionReleaseProven": True,
        "sqliteHolderContenderReleaseReopenProven": True,
        "sqliteNonWalRollbackJournalProven": True,
        "sqliteMmapDisabledProven": True,
        "concurrentProfileContenderProven": False,
        "externalOpfsWriterProven": False,
        "directV4FcntlContentionProven": False,
        "crossProcessLockingProven": False,
        "crossProfileMountLockingProven": False,
        "walOrSharedMmapProven": False,
        "normalProfilePersistenceProven": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Prove a source-selected Chromium SQLite same-process lock "
            "holder/rejection/release receipt."
        ),
        epilog=(
            "Build the isolated artifact with: buildtools/linux64/gn gen "
            "out/wasm-chrome-m7-profile-database-sqlite-lock --args='"
            + DEFAULT_GN_ARGUMENTS
            + "' --fail-on-unused-args; autoninja -C "
            "out/wasm-chrome-m7-profile-database-sqlite-lock chrome_wasm"
        ),
    )
    ...
    try:
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-profile-database-sqlite-lock-server",
            daemon=True,
        )
        server_thread.start()
        ...
        outer_profile = tempfile.TemporaryDirectory(
            prefix="chromium-wasm-m7-profile-database-sqlite-lock-"
        )
        ...
        browser_stderr_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-profile-database-sqlite-lock-browser-stderr",
            daemon=True,
        )
