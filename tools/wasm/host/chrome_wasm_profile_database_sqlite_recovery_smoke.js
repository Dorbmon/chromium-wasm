...
// Three fresh outer documents prove one intentionally bounded SQLite
// rollback-journal recovery boundary. Chromium owns profile storage and
// database work; the host receives only runner-escrowed bootstrap values and
// fixed redacted receipts. It never reads OPFS or a database. The third module
// accepts only a stable A or B SQLite value across two independently reopened
// full-integrity checks. The interruption VFS forwards a real main-database
// xSync before issuing the controlled abort. That does not prove physical
// crash or power-loss behavior, directory durability, cross-store atomicity,
// general profile persistence, or M7 completion.

const HOST_PROTOCOL = 1;
const CASE = "chrome_profile_database_sqlite_recovery_m7";
const SCOPE =
    "same-origin-three-outer-documents-chrome-wasm-m7-profile-database-bounded-sqlite-rollback-journal-main-db-sync-recovery";
const PRODUCT_MODULE_NAME =
    "chrome_wasm_m7_profile_database_sqlite_recovery_test";
...
const GENERIC_DATABASE_PHASES = Object.freeze([
  "task-post", "task-started", "sqlite-write", "task-complete",
]);
const INTERRUPTION_PHASE = "sqlite-write-main-db-sync-returned";
...
const RECOVERED_SQLITE_VALUES = Object.freeze(["a", "b"]);
...
function modeForOrdinal(ordinal) {
  ...
  if (ordinal === 2) return "interrupt-sqlite-write-b";
  if (ordinal === 3) return "recover-sqlite-write-b";
...
function expectedCleanMarkers(ordinal, tokenEvidence) {
  if (ordinal === 1) {
    return Object.freeze([
      ...
      `${M7_MARKER_PREFIX}SQLITE_RECOVERY_SEED_A_FULL_INTEGRITY_OK ` +
          `sha256=${tokenEvidence.tokenA}`,
...
function expectedInterruptionMarkers(tokenEvidence) {
  return Object.freeze([
    `${M7_MARKER_PREFIX}READY`,
    RECOVERY_LEASE_REACQUIRED_MARKER,
    `${M7_MARKER_PREFIX}SQLITE_RECOVERY_READ_A_OK ` +
        `sha256=${tokenEvidence.tokenA}`,
    `${M7_MARKER_PREFIX}SQLITE_ROLLBACK_JOURNAL_COMMIT_B_ARMED ` +
        `sha256=${tokenEvidence.tokenB}`,
  ]);
}
...
function fixedRecoveredSqliteValue(text) {
  const prefix = `${M7_MARKER_PREFIX}SQLITE_RECOVERY_`;
  if (typeof text !== "string" || !text.startsWith(prefix)) return null;
  const suffix = "_FULL_INTEGRITY_OK sha256=";
  ...
  return RECOVERED_SQLITE_VALUES.includes(value) && SHA256_RE.test(digest) ?
...
class ChromeWasmProfileDatabaseSqliteRecoveryHost {
  #activeRun = null;
  #artifact;
  #bootstrap;
  ...
  #scrubCapturedFields() {
    ...
    if (this.#run !== null) {
      ...
      this.#run.recoveredSqliteValue = null;
    }
  }
  ...
  #cleanMarkersComplete(run) {
    ...
    return run.markerSequenceAccepted && run.markerIndex === expected.length &&
        (run.ordinal !== 3 ||
         RECOVERED_SQLITE_VALUES.includes(run.recoveredSqliteValue));
  }
  ...
  #runSnapshot() {
    ...
    return {
      ...
      recoveredSqliteValue: run.recoveredSqliteValue,
...
export async function runChromeWasmProfileDatabaseSqliteRecoveryFromQuery() {
  const context = parseStaticContext();
  const root = document.querySelector(
      "#m7-profile-database-sqlite-recovery-root");
  const canvas = document.querySelector(
      "#m7-profile-database-sqlite-recovery-canvas");
  const status = document.querySelector(
      "#m7-profile-database-sqlite-recovery-status");
  if (!(root instanceof HTMLElement) || !(canvas instanceof HTMLCanvasElement) ||
      !(status instanceof HTMLElement)) {
    throw new Error("recovery page is missing required elements");
  }
  renderVersions(document.querySelector(
      "#m7-profile-database-sqlite-recovery-versions"), context.versions);
  ...
  const host = new ChromeWasmProfileDatabaseSqliteRecoveryHost(
  ...
  try {
    ...
    status.textContent = result.status === "seeded" ?
        "bounded SQLite recovery seed complete" :
        result.status === "interrupted" ?
        "bounded SQLite recovery interruption complete" :
        result.status === "recovered" ?
            "bounded SQLite recovery receipt complete" :
        "SQLite recovery probe failed; details suppressed";
