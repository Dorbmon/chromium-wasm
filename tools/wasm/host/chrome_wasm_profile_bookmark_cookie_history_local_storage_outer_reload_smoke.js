...
class ProfileBookmarkCookieHistoryLocalStorageOuterReloadHost {
...
export async function runChromeWasmProfileBookmarkCookieHistoryLocalStorageOuterReloadFromQuery() {
  const context = parseStaticContext();
  const root = document.querySelector("#m7-profile-bookmark-cookie-history-local-storage-outer-reload-root");
  const canvas = document.querySelector(
      "#m7-profile-bookmark-cookie-history-local-storage-outer-reload-canvas");
  const status = document.querySelector(
      "#m7-profile-bookmark-cookie-history-local-storage-outer-reload-status");
  if (!(root instanceof HTMLElement) || !(canvas instanceof HTMLCanvasElement) ||
      !(status instanceof HTMLElement)) {
    throw new Error("outer-reload page is missing required elements");
  }
  renderVersions(document.querySelector(
      "#m7-profile-bookmark-cookie-history-local-storage-outer-reload-versions"), context.versions);
  ...
  const host = new ProfileBookmarkCookieHistoryLocalStorageOuterReloadHost(
