...
// One-module Chromium SQLite two-connection holder/contender/release receipt.
// Chromium owns the database, its profile test lifecycle, and SQLite's normal
// rollback-journal locking path. This page verifies immutable delivery and
// lifecycle only; it has no storage, lock, native-call, or Wasm-data access.

const HOST_PROTOCOL = 1;
const CASE = "chrome_profile_database_sqlite_lock_m7";
const SCOPE =
...
const EXPECTED_MARKER_PREFIXES = Object.freeze([
  ...
  `${M7_MARKER_PREFIX}SQLITE_LOCK_HOLDER_WRITE_A_ACCEPTED sha256=`,
  `${M7_MARKER_PREFIX}SQLITE_LOCK_CONTENDER_BUSY`,
  `${M7_MARKER_PREFIX}SQLITE_LOCK_RELEASE_REOPEN_A_INTEGRITY_OK sha256=`,
  `${M7_MARKER_PREFIX}SQLITE_LOCK_POST_RELEASE_WRITE_READ_B_INTEGRITY_OK sha256=`,
...
class ChromeWasmProfileDatabaseSqliteLockHost {
  #artifact;
  ...
  constructor(canvas, status, context) {
    if (!(canvas instanceof HTMLCanvasElement) || !(status instanceof HTMLElement)) {
      throw new Error("SQLite lock smoke page is invalid");
    }
  ...
  #observeText(value, trackAcrossCallbacks = false) {
    if (typeof value !== "string" || this.#rawTokenA === null ||
        this.#rawTokenB === null) {
      return;
    }
    const candidate = trackAcrossCallbacks ? this.#rawTokenTail + value : value;
    if (candidate.includes(this.#rawTokenA) || candidate.includes(this.#rawTokenB)) {
  ...
  #expectedMarkers() {
   if (this.#tokenADigest === null || this.#tokenBDigest === null) return [];
   return [
     EXPECTED_MARKER_PREFIXES[0],
      `${EXPECTED_MARKER_PREFIXES[1]}${this.#tokenADigest}`,
     EXPECTED_MARKER_PREFIXES[2],
      `${EXPECTED_MARKER_PREFIXES[3]}${this.#tokenADigest}`,
      `${EXPECTED_MARKER_PREFIXES[4]}${this.#tokenBDigest}`,
      `${EXPECTED_MARKER_PREFIXES[5]}${this.#tokenBDigest}`,
      `${EXPECTED_MARKER_PREFIXES[6]}${this.#tokenBDigest}`,
      EXPECTED_MARKER_PREFIXES[7],
   ];
 }
  ...
  async #prepareBootstrap() {
    ...
    if (!hasExactFields(
            payload,
            ["protocol", "case", "scope", "tokenA", "tokenADigest", "tokenB",
             "tokenBDigest"]) ||
        ...
        !SHA256_RE.test(payload.tokenADigest) || typeof payload.tokenB !== "string" ||
        !SHA256_RE.test(payload.tokenB) || typeof payload.tokenBDigest !== "string" ||
        !SHA256_RE.test(payload.tokenBDigest) || payload.tokenA === payload.tokenB) {
      throw new Error("bootstrap payload is invalid");
    }
    const [tokenADigest, tokenBDigest] = await Promise.all([
      sha256Hex(new TextEncoder().encode(payload.tokenA), "database token A"),
      sha256Hex(new TextEncoder().encode(payload.tokenB), "database token B"),
    ]);
    if (tokenADigest !== payload.tokenADigest ||
        tokenBDigest !== payload.tokenBDigest) {
      throw new Error("bootstrap digest is invalid");
    }
    this.#rawTokenA = payload.tokenA;
    this.#rawTokenB = payload.tokenB;
    this.#tokenADigest = tokenADigest;
    this.#tokenBDigest = tokenBDigest;
  }
  ...
  #startModule() {
    if (this.#factory === null || this.#rawTokenA === null ||
        this.#rawTokenB === null || this.#wasmBinary === null ||
    ...
    try {
      result = this.#factory({
        arguments: [
          ...
          `--wasm-profile-database-token-a=${this.#rawTokenA}`,
          `--wasm-profile-database-token-b=${this.#rawTokenB}`,
        ],
        canvas: this.#canvas,
        locateFile(path) {
  ...
  #result(status) {
    ...
    return {
      ...
      tokenEvidence: {
        algorithm: "SHA-256",
        tokenA: this.#tokenADigest,
        tokenB: this.#tokenBDigest,
      ...
      sameProcessSqliteLockContentionReleaseProven: passed,
...
export async function runChromeWasmProfileDatabaseSqliteLockFromQuery() {
  let host = null;
  try {
    const context = parseContext();
    const canvas = document.querySelector("#m7-profile-database-sqlite-lock-canvas");
    const status = document.querySelector("#m7-profile-database-sqlite-lock-status");
    const versions = document.querySelector("#m7-profile-database-sqlite-lock-versions");
    if (!(canvas instanceof HTMLCanvasElement) || !(status instanceof HTMLElement) ||
    ...
    host = new ChromeWasmProfileDatabaseSqliteLockHost(canvas, status, context);
    ...
    const status = document.querySelector(
        "#m7-profile-database-sqlite-lock-status");
