...
// Two-outer-document aggregate preference, CookieManager, HistoryService, and
// renderer LocalStorage handoff witness. Chromium owns every profile
// operation, close fence, the one shared backend drain in each fresh module,
// and both protocol lease receipts. The host only supplies runner-escrowed
// arguments and parses exact stderr. It never opens OPFS, Web Locks, DOM
// storage, Wasm memory, or a native export. This is orderly-reload evidence,
// not crash/power-loss, normal-navigation History/UI, full-profile, general
// Cookie, or general StoragePartition persistence.

const HOST_PROTOCOL = 1;
const CASE = "chrome_profile_cookie_history_local_storage_two_outer_document_reload_m7";
const SCOPE =
    ...
    "orderly-reload-only";
const PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_cookie_history_local_storage_test";
...
const BOOTSTRAP_FIELDS = Object.freeze([
  ...
  "preferenceTokenB", "localStorageToken", "preferenceTokenADigest",
  "preferenceTokenBDigest", "localStorageTokenDigest",
]);
...
const TOKEN_EVIDENCE_FIELDS = Object.freeze([
  "algorithm", "preferenceA", "preferenceB", "localStorage", "distinct",
...
const RUN_FIELDS = Object.freeze([
  ...
  "loaderIdentity", "preferenceLeaseReleasedMarkerObserved",
...
class ProfileCookieHistoryLocalStorageOuterReloadHost {
  ...
  captureOutput(run, destination, line) {
    ...
    appendOutput(destination,
                 ...
                 exactMarker);
    if (!text.includes(M7_PREFS_MARKER_PREFIX) &&
        !text.includes(M7_LOCAL_STORAGE_MARKER_PREFIX)) return;
    ...
    if (text === `${M7_LOCAL_STORAGE_MARKER_PREFIX}LEASE_RELEASED`) {
      ...
      run.sharedDrainReceiptsAccepted =
          ...
          run.markers.at(-2) === `${M7_PREFS_MARKER_PREFIX}LEASE_RELEASED`;
    }
      ...
      return result.protocol === HOST_PROTOCOL && result.case === CASE &&
          ...
          run.stderr.filter((line) =>
            line.startsWith(M7_PREFS_MARKER_PREFIX) ||
              line.startsWith(M7_LOCAL_STORAGE_MARKER_PREFIX)).length ===
                  expected.length &&
...
export async function runChromeWasmProfileCookieHistoryLocalStorageOuterReloadFromQuery() {
  const context = parseStaticContext();
  const root = document.querySelector("#m7-profile-cookie-history-local-storage-outer-reload-root");
  const canvas = document.querySelector(
      "#m7-profile-cookie-history-local-storage-outer-reload-canvas");
  const status = document.querySelector(
      "#m7-profile-cookie-history-local-storage-outer-reload-status");
  if (!(root instanceof HTMLElement) || !(canvas instanceof HTMLCanvasElement) ||
      !(status instanceof HTMLElement)) {
    throw new Error("outer-reload page is missing required elements");
  }
  renderVersions(document.querySelector(
      "#m7-profile-cookie-history-local-storage-outer-reload-versions"), context.versions);
  ...
  const host = new ProfileCookieHistoryLocalStorageOuterReloadHost(
