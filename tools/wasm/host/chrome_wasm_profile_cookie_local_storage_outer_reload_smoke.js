...
// Two-outer-document aggregate preference, CookieManager, and renderer
// LocalStorage handoff witness. Chromium owns every profile operation, close
// fence, the one shared backend drain in each fresh module, and both protocol
// lease receipts. The host only supplies runner-escrowed arguments and parses
// exact stderr. It never opens OPFS, Web Locks, DOM storage, Wasm memory, or a
// native export. This is orderly-reload evidence, not crash/power-loss,
// full-profile, general Cookie, or general StoragePartition persistence.

const HOST_PROTOCOL = 1;
const CASE = "chrome_profile_cookie_local_storage_two_outer_document_reload_m7";
const SCOPE =
...
const PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_cookie_local_storage_test";
...
function expectedMarkers(ordinal, tokenEvidence) {
  ...
  if (ordinal === 2) {
    return Object.freeze([
      ...
      `${M7_PREFS_MARKER_PREFIX}COOKIE_BACKEND_CLOSED`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}READY`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}RENDERER_REOPEN_READ_OK sha256=${tokenEvidence.localStorage}`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}ON_DISK_COMMIT_OK sha256=${tokenEvidence.localStorage}`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}DB_CLOSE_OK sha256=${tokenEvidence.localStorage}`,
...
class ProfileCookieLocalStorageOuterReloadHost {
...
export async function runChromeWasmProfileCookieLocalStorageOuterReloadFromQuery() {
  const context = parseStaticContext();
  const root = document.querySelector("#m7-profile-cookie-local-storage-outer-reload-root");
  const canvas = document.querySelector("#m7-profile-cookie-local-storage-outer-reload-canvas");
  const status = document.querySelector("#m7-profile-cookie-local-storage-outer-reload-status");
  if (!(root instanceof HTMLElement) || !(canvas instanceof HTMLCanvasElement) ||
      !(status instanceof HTMLElement)) {
    throw new Error("outer-reload page is missing required elements");
  }
  renderVersions(document.querySelector(
      "#m7-profile-cookie-local-storage-outer-reload-versions"), context.versions);
  ...
  const host = new ProfileCookieLocalStorageOuterReloadHost(canvas, context, bootstrap, receipt);
  let result;
  try {
