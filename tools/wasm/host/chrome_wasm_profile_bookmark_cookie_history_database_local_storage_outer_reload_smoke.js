...
// HistoryService, renderer LocalStorage, and SQLite/LevelDB handoff witness.
// Chromium owns
// every profile operation, close fence, the one shared backend drain in each
// fresh module, and all three protocol lease receipts. The host only supplies
...
// StoragePartition persistence, cross-store atomicity, or database cleanup.

const HOST_PROTOCOL = 1;
const CASE = "chrome_profile_bookmark_cookie_history_database_local_storage_three_outer_document_reload_m7";
const SCOPE =
    "same-origin-three-outer-documents-chrome-wasm-m7-preferences-bookmark-model-cookie-manager-" +
    "history-service-renderer-local-storage-sqlite-leveldb-one-shared-drain-per-module-" +
    "orderly-reload-only";
const PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_bookmark_cookie_history_database_local_storage_test";
const M7_PREFS_MARKER_PREFIX = "CHROMIUM_WASM_M7_PREFS:";
const M7_LOCAL_STORAGE_MARKER_PREFIX = "CHROMIUM_WASM_M7_LOCAL_STORAGE:";
...
const BOOTSTRAP_FIELDS = Object.freeze([
  "protocol", "case", "scope", "ordinal", "mode", "preferenceTokenA",
  "preferenceTokenB", "localStorageToken", "databaseTokenA",
  "databaseTokenB", "preferenceTokenADigest", "preferenceTokenBDigest",
  "localStorageTokenDigest", "databaseTokenADigest", "databaseTokenBDigest",
]);
...
const TOKEN_EVIDENCE_FIELDS = Object.freeze([
  "algorithm", "preferenceA", "preferenceB", "localStorage", "databaseA",
  "databaseB", "distinct", "rawTokensExcluded", "rawTokenLeakDetected",
...
const RUN_FIELDS = Object.freeze([
  ...
  "factoryError", "factorySettled", "freshLoaderImport", "freshModuleObject",
  "loaderIdentity", "bookmarkTransportModeFeatureDisabled",
  "preferenceLeaseReleasedMarkerObserved", "databaseLeaseReleasedMarkerObserved",
  "localStorageLeaseReleasedMarkerObserved", "sharedDrainReceiptsAccepted",
  "markerCount",
...
function appendOutput(destination, value, exactMarker) {
  ...
  const ordinary = destination.findIndex(
      (line) => !line.startsWith(M7_PREFS_MARKER_PREFIX) &&
          !line.startsWith(M7_LOCAL_STORAGE_MARKER_PREFIX) &&
          !line.startsWith(M7_DATABASE_MARKER_PREFIX));
  if (ordinary !== -1) {
...
function expectedMarkers(ordinal, tokenEvidence) {
  if (ordinal === 1) {
    return Object.freeze([
      ...
      `${M7_PREFS_MARKER_PREFIX}WRITE_ACCEPTED sha256=${tokenEvidence.preferenceA}`,
      `${M7_PREFS_MARKER_PREFIX}BROWSER_SMOKE_CLOSED`,
      `${M7_PREFS_MARKER_PREFIX}BOOKMARK_A_WRITE_FLUSHED ` +
          `sha256=${tokenEvidence.preferenceA}`,
      `${M7_PREFS_MARKER_PREFIX}BOOKMARK_MODEL_CLOSED`,
      `${M7_PREFS_MARKER_PREFIX}COOKIE_A_WRITE_FLUSHED sha256=${tokenEvidence.preferenceA}`,
      `${M7_PREFS_MARKER_PREFIX}COOKIE_BACKEND_CLOSED`,
      `${M7_PREFS_MARKER_PREFIX}HISTORY_A_WRITE_ACCEPTED`,
      `${M7_PREFS_MARKER_PREFIX}HISTORY_BACKEND_CLOSED`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}READY`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}RENDERER_WRITE_OK sha256=${tokenEvidence.localStorage}`,
      ...
      `${M7_DATABASE_MARKER_PREFIX}SQLITE_WRITE_ACCEPTED ` +
          `sha256=${tokenEvidence.databaseA}`,
      `${M7_DATABASE_MARKER_PREFIX}LEVELDB_WRITE_ACCEPTED ` +
          `sha256=${tokenEvidence.databaseA}`,
      `${M7_DATABASE_MARKER_PREFIX}DATABASES_CLOSED ` +
          `sha256=${tokenEvidence.databaseA}`,
      `${M7_PREFS_MARKER_PREFIX}FENCE_OK sha256=${tokenEvidence.preferenceA}`,
      `${M7_DATABASE_MARKER_PREFIX}FENCE_OK sha256=${tokenEvidence.databaseA}`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}FENCE_OK sha256=${tokenEvidence.localStorage}`,
      `${M7_PREFS_MARKER_PREFIX}LEASE_RELEASED`,
      `${M7_DATABASE_MARKER_PREFIX}LEASE_RELEASED`,
  ...
  if (ordinal === 2) {
    return Object.freeze([
      `${M7_PREFS_MARKER_PREFIX}READY`,
      `${M7_PREFS_MARKER_PREFIX}READ_A_OK sha256=${tokenEvidence.preferenceA}`,
      `${M7_PREFS_MARKER_PREFIX}WRITE_ACCEPTED sha256=${tokenEvidence.preferenceB}`,
      `${M7_PREFS_MARKER_PREFIX}BROWSER_SMOKE_CLOSED`,
      `${M7_PREFS_MARKER_PREFIX}BOOKMARK_A_READ_OK ` +
          `sha256=${tokenEvidence.preferenceA}`,
      `${M7_PREFS_MARKER_PREFIX}BOOKMARK_B_WRITE_FLUSHED ` +
          `sha256=${tokenEvidence.preferenceB}`,
      `${M7_PREFS_MARKER_PREFIX}BOOKMARK_MODEL_CLOSED`,
      `${M7_PREFS_MARKER_PREFIX}COOKIE_A_READ_OK sha256=${tokenEvidence.preferenceA}`,
      `${M7_PREFS_MARKER_PREFIX}COOKIE_B_WRITE_FLUSHED sha256=${tokenEvidence.preferenceB}`,
      `${M7_PREFS_MARKER_PREFIX}COOKIE_BACKEND_CLOSED`,
      `${M7_PREFS_MARKER_PREFIX}HISTORY_A_READ_OK`,
      `${M7_PREFS_MARKER_PREFIX}HISTORY_B_WRITE_ACCEPTED`,
      `${M7_PREFS_MARKER_PREFIX}HISTORY_BACKEND_CLOSED`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}READY`,
      ...
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}DB_CLOSE_OK sha256=${tokenEvidence.localStorage}`,
      `${M7_DATABASE_MARKER_PREFIX}READY`,
      `${M7_DATABASE_MARKER_PREFIX}SQLITE_READ_A_OK ` +
          `sha256=${tokenEvidence.databaseA}`,
      `${M7_DATABASE_MARKER_PREFIX}LEVELDB_READ_A_OK ` +
          `sha256=${tokenEvidence.databaseA}`,
      `${M7_DATABASE_MARKER_PREFIX}SQLITE_WRITE_ACCEPTED ` +
          `sha256=${tokenEvidence.databaseB}`,
      `${M7_DATABASE_MARKER_PREFIX}LEVELDB_WRITE_ACCEPTED ` +
          `sha256=${tokenEvidence.databaseB}`,
      `${M7_DATABASE_MARKER_PREFIX}DATABASES_CLOSED ` +
          `sha256=${tokenEvidence.databaseB}`,
      `${M7_PREFS_MARKER_PREFIX}FENCE_OK sha256=${tokenEvidence.preferenceB}`,
      `${M7_DATABASE_MARKER_PREFIX}FENCE_OK sha256=${tokenEvidence.databaseB}`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}FENCE_OK sha256=${tokenEvidence.localStorage}`,
      `${M7_PREFS_MARKER_PREFIX}LEASE_RELEASED`,
      `${M7_DATABASE_MARKER_PREFIX}LEASE_RELEASED`,
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}LEASE_RELEASED`,
  ...
  if (ordinal === 3) {
    return Object.freeze([
      ...
      `${M7_LOCAL_STORAGE_MARKER_PREFIX}DB_CLOSE_OK sha256=${tokenEvidence.localStorage}`,
      `${M7_DATABASE_MARKER_PREFIX}READY`,
      `${M7_DATABASE_MARKER_PREFIX}SQLITE_READ_B_OK ` +
          `sha256=${tokenEvidence.databaseB}`,
      `${M7_DATABASE_MARKER_PREFIX}LEVELDB_READ_B_OK ` +
          `sha256=${tokenEvidence.databaseB}`,
      `${M7_DATABASE_MARKER_PREFIX}DATABASES_CLOSED ` +
          `sha256=${tokenEvidence.databaseB}`,
      `${M7_PREFS_MARKER_PREFIX}FENCE_OK sha256=${tokenEvidence.preferenceB}`,
...
async function fetchBootstrap(context) {
  ...
  const validTokenAndDigest = (token, digest) =>
    typeof token === "string" && SHA256_RE.test(token) &&
      typeof digest === "string" && SHA256_RE.test(digest);
  const absentTokenAndDigest = (token, digest) =>
    token === null && digest === null;
  if (bootstrap.protocol !== HOST_PROTOCOL || bootstrap.case !== CASE ||
      ...
      (!third ?
        (!validTokenAndDigest(bootstrap.preferenceTokenA,
                              bootstrap.preferenceTokenADigest) ||
         !validTokenAndDigest(bootstrap.databaseTokenA,
                              bootstrap.databaseTokenADigest)) :
        (!absentTokenAndDigest(bootstrap.preferenceTokenA,
                               bootstrap.preferenceTokenADigest) ||
         !absentTokenAndDigest(bootstrap.databaseTokenA,
                               bootstrap.databaseTokenADigest))) ||
      (!first ?
        (!validTokenAndDigest(bootstrap.preferenceTokenB,
                              bootstrap.preferenceTokenBDigest) ||
         !validTokenAndDigest(bootstrap.databaseTokenB,
                              bootstrap.databaseTokenBDigest)) :
        (!absentTokenAndDigest(bootstrap.preferenceTokenB,
                               bootstrap.preferenceTokenBDigest) ||
         !absentTokenAndDigest(bootstrap.databaseTokenB,
                               bootstrap.databaseTokenBDigest))) ||
      !validTokenAndDigest(bootstrap.localStorageToken,
                           bootstrap.localStorageTokenDigest)) {
    throw new Error("outer-reload bootstrap is invalid");
  }
  const presentRawTokens = [
    bootstrap.preferenceTokenA,
    bootstrap.preferenceTokenB,
    bootstrap.localStorageToken,
    bootstrap.databaseTokenA,
    bootstrap.databaseTokenB,
  ].filter((token) => token !== null);
  const presentSuppliedDigests = [
    bootstrap.preferenceTokenADigest,
    bootstrap.preferenceTokenBDigest,
    bootstrap.localStorageTokenDigest,
    bootstrap.databaseTokenADigest,
  ...
  if (presentRawTokens.length !== 3 + (second ? 2 : 0) ||
      new Set(presentRawTokens).size !== presentRawTokens.length ||
      new Set(presentSuppliedDigests).size !== presentSuppliedDigests.length) {
    throw new Error("outer-reload bootstrap token set is invalid");
  }
  ...
  const preferenceB = first ? null : await sha256Hex(
      encoder.encode(bootstrap.preferenceTokenB),
      "outer-reload preference token B");
  const localStorage = await sha256Hex(
      encoder.encode(bootstrap.localStorageToken),
      "outer-reload LocalStorage token");
  const databaseA = third ? null : await sha256Hex(
      encoder.encode(bootstrap.databaseTokenA),
      "outer-reload database token A");
  const databaseB = first ? null : await sha256Hex(
      encoder.encode(bootstrap.databaseTokenB),
      "outer-reload database token B");
  if ((!third && preferenceA !== bootstrap.preferenceTokenADigest) ||
      ...
      (first && preferenceB !== null) ||
      localStorage !== bootstrap.localStorageTokenDigest ||
      (!third && databaseA !== bootstrap.databaseTokenADigest) ||
      (third && databaseA !== null) ||
      (!first && databaseB !== bootstrap.databaseTokenBDigest) ||
      (first && databaseB !== null) ||
      new Set([preferenceA, preferenceB, localStorage, databaseA, databaseB]
          .filter((digest) => digest !== null)).size !==
          presentSuppliedDigests.length) {
    throw new Error("outer-reload bootstrap token identity is invalid");
  }
  return Object.freeze({
    mode: bootstrap.mode,
    ordinal: bootstrap.ordinal,
    rawTokens: Object.freeze({
      preferenceA: bootstrap.preferenceTokenA,
      preferenceB: bootstrap.preferenceTokenB,
      localStorage: bootstrap.localStorageToken,
      databaseA: bootstrap.databaseTokenA,
      databaseB: bootstrap.databaseTokenB,
    }),
...
class ProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadHost {
  ...
  scrubCapturedFields() {
    ...
    if (this.run !== null) {
      ...
      this.run.preferenceLeaseReleasedMarkerObserved = false;
      this.run.databaseLeaseReleasedMarkerObserved = false;
      this.run.localStorageLeaseReleasedMarkerObserved = false;
      this.run.sharedDrainReceiptsAccepted = false;
    }
  }
  ...
  recordOpaqueTokenLeak() {
    ...
    this.recordFatal("opaque-token-leak");
  }
  ...
  installBridge() {
    ...
    const bridge = Object.freeze({
      ...
      reportFatal(message) {
        note();
        host.safeText(typeof message === "string" ? message : "", true);
  ...
  async prepareFactory() {
    ...
    this.loaderIdentity = randomHex(MODULE_ID_BYTES);
  ...
  newRun() {
    return {
      ...
      freshLoaderImport: this.loaderIdentity !== null,
      freshModuleObject: false,
      loaderIdentity: this.loaderIdentity,
      bookmarkTransportModeFeatureDisabled: false,
      preferenceLeaseReleasedMarkerObserved: false,
      databaseLeaseReleasedMarkerObserved: false,
      localStorageLeaseReleasedMarkerObserved: false,
      sharedDrainReceiptsAccepted: false,
  ...
  captureOutput(run, destination, line) {
    ...
    const containsAcceptanceMarker =
        text.includes(M7_PREFS_MARKER_PREFIX) ||
        text.includes(M7_LOCAL_STORAGE_MARKER_PREFIX) ||
        text.includes(M7_DATABASE_MARKER_PREFIX);
    const containsDatabaseDiagnostic =
        text.includes(M7_DATABASE_PHASE_PREFIX) ||
        text.includes(M7_ABORT_PC_PREFIX);
    const expected = destination === run.stderr && this.activeRun === run ?
        run.expectedMarkers[run.markerIndex] : null;
    const exactMarker = text === expected &&
        (text.startsWith(M7_PREFS_MARKER_PREFIX) ||
         text.startsWith(M7_LOCAL_STORAGE_MARKER_PREFIX) ||
         text.startsWith(M7_DATABASE_MARKER_PREFIX));
    appendOutput(destination,
                 exactMarker ? text : SUPPRESSED_NATIVE_OUTPUT,
                 exactMarker);
    if (containsDatabaseDiagnostic) {
      run.markerSequenceAccepted = false;
      this.recordFatal("database-diagnostic-telemetry");
      return;
    }
    if (!containsAcceptanceMarker) return;
    if (destination !== run.stderr || this.activeRun !== run) {
      this.recordFatal("marker-outside-active-stderr");
      return;
    }
    if (text.startsWith(`${M7_PREFS_MARKER_PREFIX}FAIL stage=`) ||
        text.startsWith(`${M7_LOCAL_STORAGE_MARKER_PREFIX}FAIL stage=`) ||
        text.startsWith(`${M7_DATABASE_MARKER_PREFIX}FAIL stage=`)) {
    ...
    ++run.markerIndex;
    if (text === `${M7_PREFS_MARKER_PREFIX}LEASE_RELEASED`) {
      run.preferenceLeaseReleasedMarkerObserved = true;
    }
    if (text === `${M7_DATABASE_MARKER_PREFIX}LEASE_RELEASED`) {
      run.databaseLeaseReleasedMarkerObserved = true;
    }
    if (text === `${M7_LOCAL_STORAGE_MARKER_PREFIX}LEASE_RELEASED`) {
      run.localStorageLeaseReleasedMarkerObserved = true;
      run.sharedDrainReceiptsAccepted =
          run.preferenceLeaseReleasedMarkerObserved &&
          run.databaseLeaseReleasedMarkerObserved &&
          run.markers.length === run.expectedMarkers.length &&
          run.markers.at(-3) === `${M7_PREFS_MARKER_PREFIX}LEASE_RELEASED` &&
          run.markers.at(-2) === `${M7_DATABASE_MARKER_PREFIX}LEASE_RELEASED`;
    }
  ...
  markersComplete(run) {
    return run.markerSequenceAccepted &&
        ...
        run.preferenceLeaseReleasedMarkerObserved &&
        run.databaseLeaseReleasedMarkerObserved &&
        run.localStorageLeaseReleasedMarkerObserved &&
        run.sharedDrainReceiptsAccepted;
  }
  ...
  startRun() {
    ...
    if (run.ordinal === 1) {
      moduleArguments = [
        ...
        `--wasm-profile-preferences-token-a=${this.rawTokens.preferenceA}`,
        ...
        `--wasm-profile-database-token-a=${this.rawTokens.databaseA}`,
      ];
      ...
      moduleArguments = [
        ...
        `--wasm-profile-database-token-a=${this.rawTokens.databaseA}`,
        `--wasm-profile-database-token-b=${this.rawTokens.databaseB}`,
      ];
    ...
    run.bookmarkTransportModeFeatureDisabled = moduleArguments.filter(
        (argument) => argument ===
            "--disable-features=SyncEnableBookmarksInTransportMode").length === 1;
    if (!run.bookmarkTransportModeFeatureDisabled) {
      this.recordFatal("bookmark-transport-mode-disablement-invalid");
      return;
    }
    const host = this;
  ...
  runSnapshot() {
    ...
    return {
      ...
      loaderIdentity: run.loaderIdentity,
      bookmarkTransportModeFeatureDisabled:
          run.bookmarkTransportModeFeatureDisabled,
      preferenceLeaseReleasedMarkerObserved:
          run.preferenceLeaseReleasedMarkerObserved,
      databaseLeaseReleasedMarkerObserved:
          run.databaseLeaseReleasedMarkerObserved,
      localStorageLeaseReleasedMarkerObserved:
          run.localStorageLeaseReleasedMarkerObserved,
      sharedDrainReceiptsAccepted: run.sharedDrainReceiptsAccepted,
  ...
  strictPass(result) {
    try {
      ...
      return result.protocol === HOST_PROTOCOL && result.case === CASE &&
          ...
          tokenEvidence.preferenceA ===
              this.bootstrap.tokenEvidence.preferenceA &&
          tokenEvidence.preferenceB ===
              this.bootstrap.tokenEvidence.preferenceB &&
          tokenEvidence.localStorage ===
              this.bootstrap.tokenEvidence.localStorage &&
          tokenEvidence.databaseA ===
              this.bootstrap.tokenEvidence.databaseA &&
          tokenEvidence.databaseB ===
              this.bootstrap.tokenEvidence.databaseB &&
          ...
          run.freshModuleObject === true &&
          run.bookmarkTransportModeFeatureDisabled === true &&
          ...
          run.databaseLeaseReleasedMarkerObserved === true &&
          run.localStorageLeaseReleasedMarkerObserved === true &&
          run.sharedDrainReceiptsAccepted === true &&
          ...
          run.stderr.filter((line) =>
            line.startsWith(M7_PREFS_MARKER_PREFIX) ||
              line.startsWith(M7_LOCAL_STORAGE_MARKER_PREFIX) ||
              line.startsWith(M7_DATABASE_MARKER_PREFIX)).length ===
                  expected.length &&
          !run.stdout.concat(run.stderr).some((line) =>
            line.includes(M7_DATABASE_PHASE_PREFIX) ||
              line.includes(M7_ABORT_PC_PREFIX)) &&
          run.loaderIdentity && MODULE_ID_RE.test(run.loaderIdentity) &&
...
export async function runChromeWasmProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadFromQuery() {
  const context = parseStaticContext();
  const root = document.querySelector("#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-root");
  const canvas = document.querySelector(
      "#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-canvas");
  const status = document.querySelector(
      "#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-status");
  if (!(root instanceof HTMLElement) || !(canvas instanceof HTMLCanvasElement) ||
      !(status instanceof HTMLElement)) {
    throw new Error("outer-reload page is missing required elements");
  }
  renderVersions(document.querySelector(
      "#m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-versions"), context.versions);
  ...
  const host = new ProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadHost(
