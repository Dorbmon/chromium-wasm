...
// Three-fresh-document, post-LevelDB-log-Sync interruption observation.  This
// is deliberately a non-gating diagnostic. Chromium owns profile storage and
// the database work; the host only receives one runner-escrowed bootstrap body
// per document and reports fixed, redacted state. It never reads OPFS or a
// database, and it never turns an observed post-Sync value into persistence or
// durable-write success.

const HOST_PROTOCOL = 1;
const CASE = "chrome_profile_database_write_interruption_m7";
const SCOPE =
    "same-origin-three-outer-documents-chrome-wasm-m7-profile-database-post-leveldb-log-sync-interruption-diagnostic-only";
...
const DIAGNOSTIC_CLEAN_MARKERS = Object.freeze([
  ...
  `${M7_MARKER_PREFIX}DIAGNOSTIC_LEASE_RELEASED`,
]);
...
function parsePositiveTimeout(value) {
  if (typeof value !== "string" || !/^[0-9]+$/.test(value)) {
    throw new Error("write-interruption timeout is invalid");
  }
...
function parseArtifactIdentity(value) {
  const artifact = requireExactFields(
      parseQueryJson(value, "write-interruption artifact identity"),
      ARTIFACT_FIELDS, "write-interruption artifact identity");
  if (artifact.artifact_delivery !== "immutable-in-memory-server-snapshot" ||
    ...
    throw new Error("write-interruption artifact identity is invalid");
  }
  return Object.freeze({
    ...
    build_config: parseByteIdentity(artifact.build_config,
                                    "write-interruption build configuration"),
    build_config_provenance: artifact.build_config_provenance,
    loader: parseByteIdentity(artifact.loader, "write-interruption loader"),
    module_name: artifact.module_name,
    wasm: parseByteIdentity(artifact.wasm, "write-interruption Wasm"),
  });
}

function parseCaptureHarnessIdentity(value) {
  const harness = requireExactFields(
      parseQueryJson(value, "write-interruption capture harness"),
      CAPTURE_HARNESS_FIELDS, "write-interruption capture harness");
  if (harness.source_snapshot_provenance !==
    ...
    throw new Error("write-interruption capture harness is invalid");
  }
  return Object.freeze({
    host_html: parseByteIdentity(harness.host_html,
                                 "write-interruption host HTML"),
    host_js: parseByteIdentity(harness.host_js,
                               "write-interruption host JavaScript"),
    runner_source: parseByteIdentity(harness.runner_source,
                                     "write-interruption runner source"),
...
function parseVersions(value) {
  const versions = requireExactFields(
      parseQueryJson(value, "write-interruption versions"),
      ["chromium", "v8", "emscripten"], "write-interruption versions");
  for (const revision of Object.values(versions)) {
    if (typeof revision !== "string" || !/^[0-9a-f]{40}$/.test(revision)) {
      throw new Error("write-interruption versions are invalid");
    }
  }
...
function parseStaticContext() {
  ...
  for (const name of query.keys()) {
    if (!allowed.has(name) || query.getAll(name).length !== 1) {
      throw new Error("write-interruption query is invalid");
    }
  }
  const resultToken = requireString(query.get("resultToken"),
                                    "write-interruption result capability");
  const session = requireString(query.get("session"),
                                "write-interruption session capability");
...
function randomHex(byteLength) {
  if (!Number.isSafeInteger(byteLength) || byteLength < 1 || !globalThis.crypto ||
    ...
    throw new Error("write-interruption random source is unavailable");
  }
...
function modeForOrdinal(ordinal) {
  ...
  if (ordinal === 3) return "observe-leveldb-write-b";
  throw new Error("write-interruption bootstrap ordinal is invalid");
}
...
function expectedCleanMarkers(ordinal, tokenEvidence) {
  ...
  throw new Error("write-interruption clean markers are invalid");
}
...
function fixedPostSyncObservation(text) {
  const prefix = `${M7_MARKER_PREFIX}LEVELDB_POST_SYNC_OBSERVATION outcome=`;
  if (typeof text !== "string" || !text.startsWith(prefix)) return null;
  const outcome = text.slice(prefix.length);
  return POST_SYNC_OBSERVATION_OUTCOMES.includes(outcome) ? outcome : null;
}

export function isExactWriteInterruptionExitStatus(value) {
  try {
...
function documentEvidence() {
  ...
  if ((navigationType !== "navigate" && navigationType !== "reload") ||
      ...
      timeOrigin <= 0) {
    throw new Error("write-interruption document evidence is invalid");
  }
...
function bootstrapUrl(context) {
  ...
  if (endpoint.origin !== location.origin) {
    throw new Error("write-interruption bootstrap endpoint is invalid");
  }
  return endpoint;
}
...
async function postBootstrapDocumentEvidence(context, receipt) {
  const document = requireExactFields(
      ...
      "write-interruption bootstrap document evidence");
...
async function fetchBootstrap(context) {
  ...
  try {
    ...
    throw new Error("write-interruption bootstrap request failed");
  }
  if (response.status !== 200 || response.url !== endpoint.href) {
    throw new Error("write-interruption bootstrap request was rejected");
  }
  requireResponseHeaders(response, "application/json", "write-interruption bootstrap");
  let value;
  try {
    value = await response.json();
  } catch (_error) {
    throw new Error("write-interruption bootstrap is invalid");
  }
...
async function parseBootstrap(value) {
  ...
  if (typeof TextEncoder !== "function") {
    throw new Error("write-interruption token encoder is unavailable");
  }
  const tokenA = await sha256Hex(
      new TextEncoder().encode(bootstrap.tokenA), "write-interruption token A");
  const tokenB = hasB ? await sha256Hex(
      new TextEncoder().encode(bootstrap.tokenB), "write-interruption token B") : null;
  if (tokenA !== bootstrap.tokenADigest ||
      (hasB && tokenB !== bootstrap.tokenBDigest)) {
    throw new Error("write-interruption bootstrap token identity is invalid");
  }
  return Object.freeze({
...
class ChromeWasmProfileDatabaseWriteInterruptionHost {
  #activeRun = null;
  #artifact;
  #bootstrap;
  ...
  constructor(canvas, context, bootstrap, documentReceipt) {
    if (!(canvas instanceof HTMLCanvasElement)) {
      throw new Error("write-interruption canvas is unavailable");
    }
  ...
  #recordFailure(code) {
    if (typeof code !== "string" || code.length === 0) {
      throw new Error("write-interruption failure code is invalid");
    }
  ...
  #scrubCapturedFields() {
    ...
    if (this.#run !== null) {
      ...
      this.#run.postSyncObservation = null;
    }
  }
  ...
  #installPermanentBridge() {
    if (globalThis.__chromiumWasmHostBridgeV1 !== undefined) {
      throw new Error("write-interruption host bridge already exists");
    }
    const host = this;
    ...
    if (globalThis.__chromiumWasmHostBridgeV1 !== bridge ||
      ...
      throw new Error("write-interruption host bridge is invalid");
    }
  ...
  async #prepareFactory() {
    ...
    if (typeof namespace.default !== "function") {
      throw new Error("write-interruption loader has no factory");
    }
  ...
  #captureOutput(run, destination, line) {
    ...
    if (containsPhase) {
      ...
      if (phase === INTERRUPTION_PHASE) {
        if (run.ordinal !== 2 || run.phaseObserved || run.markerIndex !== 3) {
        ...
        // Only doc 1 deliberately preserves its legacy diagnostic phase
        // stream. The aborted and post-Sync observation modes must expose no
        // extra phase data that could be mistaken for lifecycle completion.
    ...
    if (run.ordinal === 3) {
      const outcome = fixedPostSyncObservation(text);
      if (outcome !== null) {
        if (run.markerIndex !== 1 || run.postSyncObservation !== null) {
          this.#recordFatal("post-sync-observation-invalid");
          return;
        }
        run.postSyncObservation = outcome;
  ...
  #acceptExpectedCleanExitRejection(event, reason) {
    ...
    if (run === null || run.ordinal === 2 || run.expectedCleanExitStatusObserved ||
        !isExactWriteInterruptionExitStatus(reason) || !event ||
  ...
  #cleanMarkersComplete(run) {
    ...
    return run.markerSequenceAccepted && run.markerIndex === expected.length &&
        (run.ordinal !== 3 ||
         POST_SYNC_OBSERVATION_OUTCOMES.includes(run.postSyncObservation));
  }
  ...
  #locateFileForWasm(path) {
    if (typeof path !== "string" ||
      ...
      throw new Error("write-interruption loader requested an unexpected artifact");
    }
  ...
  #runSnapshot() {
    ...
    return {
      ...
      postSyncObservation: run.postSyncObservation,
  ...
  #statusForRun() {
    ...
    if (this.#bootstrap.ordinal === 3) return "observed";
    return "fail";
  }
  ...
  async run() {
    try {
      if (globalThis.crossOriginIsolated !== true ||
        ...
        throw new Error("write-interruption host precondition is invalid");
      }
      this.#canvas.focus({preventScroll: true});
      if (document.activeElement !== this.#canvas) {
        throw new Error("write-interruption canvas focus failed");
      }
      ...
      if (this.#run === null || !this.#run.settleComplete) {
        this.#recordFatal("write-interruption-host-timeout");
      }
      ...
      this.#recordFatal("write-interruption-host-exception");
      return this.#baseResult("fail", "details-suppressed");
    }
  }
...
function renderVersions(element, versions) {
  if (!(element instanceof HTMLElement)) {
    throw new Error("write-interruption page is missing its version element");
  }
...
function resultEndpoint(context, ordinal) {
  const endpoint = new URL(
      ...
      location.href);
  if (endpoint.origin !== location.origin) {
    throw new Error("write-interruption result endpoint is invalid");
  }
  return endpoint;
}
...
function readyEndpoint(context, ordinal) {
  const endpoint = new URL(
      ...
      location.href);
  if (endpoint.origin !== location.origin) {
    throw new Error("write-interruption ready endpoint is invalid");
  }
  return endpoint;
}
...
async function postReady(context, result) {
  ...
  }, "write-interruption ready");
}

export async function runChromeWasmProfileDatabaseWriteInterruptionFromQuery() {
  const context = parseStaticContext();
  const root = document.querySelector(
      "#m7-profile-database-write-interruption-root");
  const canvas = document.querySelector(
      "#m7-profile-database-write-interruption-canvas");
  const status = document.querySelector(
      "#m7-profile-database-write-interruption-status");
  if (!(root instanceof HTMLElement) || !(canvas instanceof HTMLCanvasElement) ||
      !(status instanceof HTMLElement)) {
    throw new Error("write-interruption page is missing required elements");
  }
  renderVersions(document.querySelector(
      "#m7-profile-database-write-interruption-versions"), context.versions);
  ...
  const host = new ChromeWasmProfileDatabaseWriteInterruptionHost(
  ...
  try {
    ...
    status.textContent = result.status === "seeded" ? "diagnostic seed complete" :
        result.status === "interrupted" ? "diagnostic interruption observed" :
        result.status === "observed" ?
            "diagnostic post-Sync value observation complete" :
        "profile database write-interruption diagnostic failed; details suppressed";
    await postPhaseResult(context, result);
    if (result.status === "fail") {
      throw new Error("write-interruption result validation failed");
    }
    ...
    if (!host.isReadyAfterResultUpload()) {
      throw new Error("write-interruption host changed after result upload");
    }
