...
"""Run the aggregate Cookie + History + renderer LocalStorage outer-reload witness.

Document one writes preference A, Cookie A, and History A, then writes one
separately escrowed token through a real renderer's ``window.localStorage``.
Only after the Browser, Cookie SQLite backend, History/Favicons SQLite
backends, renderer LocalStorage database, Preferences fence, and the one
shared V4 drain have completed may both protocol-specific ``LEASE_RELEASED``
receipts appear. The runner then issues exactly one DevTools ``Page.reload``.
Document two reads preference A, Cookie A, and History A before writing
preference B, Cookie B, and History B, and reopens the same LocalStorage token
...
crash/power-loss durability, full-profile persistence, normal navigation
history or the History UI, the general Cookie request path, or a generally
persistent StoragePartition. Raw preference A, preference B, and LocalStorage
values exist only in this process's in-memory escrow and one-shot bootstrap
bodies. They are scanned out of browser stderr and excluded from every receipt
and final result; only SHA-256 digests leave the escrow boundary.
"""
...
SENTINEL = "CHROMIUM_WASM_M7_CHROME_PROFILE_COOKIE_HISTORY_LOCAL_STORAGE_OUTER_RELOAD_DOM"
CASE = "chrome_profile_cookie_history_local_storage_two_outer_document_reload_m7"
SCOPE = (
...
PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_cookie_history_local_storage_test"
PRODUCT_GN_TARGET = "//chrome:chrome_wasm"
PRODUCT_GN_ENABLE_ARGUMENT = (
    "enable_chromium_wasm_m7_profile_cookie_history_local_storage_test=true"
)
DEFAULT_OUT_DIR = Path("out/wasm-chrome-m7-profile-cookie-history-local-storage")
...
HOST_ROOT = "/__m7_chrome_profile_cookie_history_local_storage_outer_reload__"
HOST_HTML_NAME = "chrome_wasm_profile_cookie_history_local_storage_outer_reload_smoke.html"
HOST_JS_NAME = "chrome_wasm_profile_cookie_history_local_storage_outer_reload_smoke.js"
...
M7_GN_ENABLE_ASSIGNMENT_RE = re.compile(
    r"^[ \t]*enable_chromium_wasm_m7_profile_cookie_history_local_storage_test[ \t]*="
    r"[ \t]*(true|false)[ \t]*(?:#.*)?$",
    re.MULTILINE,
)
...
class ChromeProfileCookieHistoryLocalStorageOuterReloadServer(ThreadingHTTPServer):
    daemon_threads = True
...
class ChromeProfileCookieHistoryLocalStorageOuterReloadRequestHandler(BaseHTTPRequestHandler):
    server: ChromeProfileCookieHistoryLocalStorageOuterReloadServer
...
def create_server(
    ...
    *,
...
) -> ChromeProfileCookieHistoryLocalStorageOuterReloadServer:
    ...
    artifacts = snapshot_regular_files(
        ...
        description="outer-reload profile Cookie/History/LocalStorage artifacts",
    )
    ...
    server = ChromeProfileCookieHistoryLocalStorageOuterReloadServer(
        (host, port), ChromeProfileCookieHistoryLocalStorageOuterReloadRequestHandler
    )
...
def verify_server_delivery(server: ChromeProfileCookieHistoryLocalStorageOuterReloadServer) -> None:
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = (
        diagnostics_dir
        / "chrome-profile-cookie-history-local-storage-outer-reload-m7-failure.json"
    )
    payload = {
        "schema_version": 1,
        "runner": "run_m7_chrome_profile_cookie_history_local_storage_outer_reload_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "stage": stage,
        "nonclaims": [
...
def _stop_server(
    server: ChromeProfileCookieHistoryLocalStorageOuterReloadServer | None,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run two aggregate Cookie + History + renderer LocalStorage Chrome Wasm "
            "documents separated by one real DevTools Page.reload."
        ),
        epilog=(
            "Build the normal artifact with: buildtools/linux64/gn gen "
            "out/wasm-chrome-m7-profile-cookie-history-local-storage --args='"
            + DEFAULT_GN_ARGUMENTS
            + "' --fail-on-unused-args; autoninja -C "
            "out/wasm-chrome-m7-profile-cookie-history-local-storage chrome_wasm"
        ),
    )
    ...
    server: ChromeProfileCookieHistoryLocalStorageOuterReloadServer | None = None
    ...
    try:
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-profile-cookie-history-local-storage-outer-reload-server",
            daemon=True,
        )
        server_thread.start()
        ...
        outer_profile = tempfile.TemporaryDirectory(
            prefix="chromium-wasm-m7-profile-cookie-history-local-storage-outer-reload-"
        )
        ...
        stderr_thread = threading.Thread(
            ...
            name=(
                "chromium-wasm-m7-profile-cookie-history-local-storage-"
