...
def gpu_lists_version_path() -> Path:
    configured_path = PurePosixPath("gpu/config/gpu_lists_version.h")
    repository_root = REPO_ROOT.resolve()
    path = REPO_ROOT.joinpath(*configured_path.parts)
    try:
        path.parent.resolve().relative_to(repository_root)
    ...
    if path.is_symlink():
        raise M0Error("GPU lists version header path must not be a symlink")
    return path


def verify_gpu_lists_version_deps_hook(manifest: dict[str, object]) -> None:
    chromium = manifest["chromium"]
    assert isinstance(chromium, dict)
    revision = str(chromium["revision"])
    if not is_lower_hex(revision, 40):
        raise M0Error(
            "Chromium revision for GPU lists version must be a lowercase "
            "40-character Git hash"
        )

    gpu_lists_version_path()
    deps_lines = (REPO_ROOT / "DEPS").read_text(
        encoding="utf-8"
    ).splitlines()
    marker = "'name': 'gpu_lists_version',"
    marker_lines = [
        index
        for index, line in enumerate(deps_lines)
        if line.strip() == marker
    ...
    if len(marker_lines) != 1:
        raise M0Error(
            "Chromium DEPS must contain exactly one gpu_lists_version hook"
        )
    start = marker_lines[0]
    while start >= 0 and deps_lines[start].strip() != "{":
        start -= 1
    ...
    if start < 0 or end == len(deps_lines):
        raise M0Error("Chromium DEPS gpu_lists_version hook is malformed")
    hook_text = "\n".join(deps_lines[start : end + 1])
    ...
    for expected in expected_fields:
        if hook_text.count(expected) != 1:
            raise M0Error(
                "Chromium DEPS gpu_lists_version hook is missing "
                f"{expected}"
            )

    try:
        hook = ast.literal_eval(hook_text)
    except (SyntaxError, ValueError) as exc:
        raise M0Error("Chromium DEPS gpu_lists_version hook is malformed") from exc
    ...
    if not isinstance(hook, dict) or hook.get("action") != expected_action:
        raise M0Error("Chromium DEPS gpu_lists_version action mismatch")


def gpu_lists_version_contents(manifest: dict[str, object]) -> str:
    chromium = manifest["chromium"]
    assert isinstance(chromium, dict)
    revision = str(chromium["revision"])
    return (
...
def ensure_gpu_lists_version(
    manifest: dict[str, object], *, install: bool
) -> None:
    verify_gpu_lists_version_deps_hook(manifest)
    header_path = gpu_lists_version_path()
    header = gpu_lists_version_contents(manifest)

    if install:
        header_path.parent.mkdir(parents=True, exist_ok=True)
        if header_path.exists() and not header_path.is_file():
            raise M0Error(
                "generated GPU lists version header path is not a regular file"
            )
        if (
            not header_path.exists()
            or header_path.read_text(encoding="utf-8") != header
    ...
    require_equal(
        "generated GPU lists version header",
        header_path.read_text(encoding="utf-8"),
        header,
    )
...
def main() -> int:
    ...
    try:
        ...
        if args.profile == "m3":
            ...
            ensure_gpu_lists_version(manifest, install=install)
