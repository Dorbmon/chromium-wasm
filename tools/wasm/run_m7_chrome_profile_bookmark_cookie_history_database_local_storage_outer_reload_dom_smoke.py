...
"""Run the five-store profile outer-reload persistence witness.

Document one writes preference A through the Bookmark, Cookie, and History
aggregate, database A through real SQLite and LevelDB handles, and one
separately escrowed token through a real renderer's ``window.localStorage``.
Only after the Browser, BookmarkModel, Cookie SQLite backend, History/Favicons
SQLite backends, database handles, renderer LocalStorage database, all three
fences, and the one shared V5 drain have completed may the three ordered
``LEASE_RELEASED`` receipts appear. The runner then issues
the first DevTools ``Page.reload``. Document two reads preference A, Bookmark
A, Cookie A, and History A before writing preference B, Bookmark B, Cookie B,
and History B, reads database A before writing database B, and reopens the same
LocalStorage token through Blink. After the same ordered close/fence/drain
boundary, a second ``Page.reload`` creates document three. It reopens
preference B, Bookmark B, Cookie B, both History rows, database B, and renderer
LocalStorage; removes Bookmark B with a result-bearing cleanup flush; and
reaches the same shared drain again.

The retained CDP client requires both reloads to keep the root frame and
replace its loader. The three receipts must also carry increasing
``timeOrigin`` values, ``navigate`` then ``reload``/``reload`` navigation
types, pairwise-distinct loader identities, and pairwise-distinct Wasm module
identities. No host JavaScript self-navigates.

This is a source-selected orderly-reload witness only. It proves Bookmark B
reopen plus a returned cleanup write, not a fourth reopen of that cleanup or
the full Bookmark service. It does not prove crash/power-loss durability,
full-profile persistence, normal navigation history or the History UI, the
general Cookie request path, crash/interrupted-write database recovery, or a
generally persistent StoragePartition. The five raw values exist only in this
process's in-memory escrow and one-shot bootstrap bodies. They are scanned out
of browser stderr and excluded from every receipt and final result; only
SHA-256 digests leave the escrow boundary.
"""
...
SENTINEL = "CHROMIUM_WASM_M7_CHROME_PROFILE_BOOKMARK_COOKIE_HISTORY_DATABASE_LOCAL_STORAGE_OUTER_RELOAD_DOM"
CASE = "chrome_profile_bookmark_cookie_history_database_local_storage_three_outer_document_reload_m7"
SCOPE = (
...
PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_bookmark_cookie_history_database_local_storage_test"
...
DEFAULT_OUT_DIR = Path("out/wasm-chrome-m7-profile-bookmark-cookie-history-database-local-storage")
...
HOST_ROOT = "/__m7_chrome_profile_bookmark_cookie_history_database_local_storage_outer_reload__"
HOST_HTML_NAME = "chrome_wasm_profile_bookmark_cookie_history_database_local_storage_outer_reload_smoke.html"
HOST_JS_NAME = "chrome_wasm_profile_bookmark_cookie_history_database_local_storage_outer_reload_smoke.js"
...
M7_GN_ENABLE_ASSIGNMENT_RE = re.compile(
    r"^[ \t]*enable_chromium_wasm_m7_profile_bookmark_cookie_history_database_local_storage_test[ \t]*="
    r"[ \t]*(?:\r?\n[ \t]+)?(true|false)[ \t]*(?:#.*)?$",
    re.MULTILINE,
)
...
class TokenEscrow:
    ...
    database_a: str = field(repr=False)
    database_b: str = field(repr=False)
    local_storage: str = field(repr=False)
    preference_a_digest: str
    preference_b_digest: str
    database_a_digest: str
    database_b_digest: str
...
def drain_browser_stderr(
    ...
    for line in stream:
        ...
        combined = opaque_token_tail + normalized
        if any(
            token in combined
            for token in (
                escrow.preference_a,
        ...
        opaque_token_tail = combined[-OPAQUE_TOKEN_TAIL_CHARS:]
...
def new_token_escrow() -> TokenEscrow:
    ...
    database_a = secrets.token_hex(32)
    database_b = secrets.token_hex(32)
    local_storage = secrets.token_hex(32)
    while len(
        {preference_a, preference_b, database_a, database_b, local_storage}
    ) != 5:
        preference_b = secrets.token_hex(32)
        database_a = secrets.token_hex(32)
        database_b = secrets.token_hex(32)
    ...
    return TokenEscrow(
        ...
        database_a_digest=_sha256_text(database_a),
        database_b_digest=_sha256_text(database_b),
...
class OuterReloadSession:
    ...
    def __init__(self, result_token: str, session: str, escrow: TokenEscrow):
        if (
            ...
            or not SHA256_RE.fullmatch(escrow.database_a)
            or not SHA256_RE.fullmatch(escrow.database_b)
            ...
            or escrow.database_a_digest != _sha256_text(escrow.database_a)
            or escrow.database_b_digest != _sha256_text(escrow.database_b)
    ...
    def bootstrap_payload(self, session: str) -> dict[str, object] | None:
        ...
        return {
            ...
            "preferenceTokenA": (
                None if ordinal == 3 else self.escrow.preference_a
            ),
            "preferenceTokenB": (
                None if ordinal == 1 else self.escrow.preference_b
            ),
            "databaseTokenA": None if ordinal == 3 else self.escrow.database_a,
            "databaseTokenB": None if ordinal == 1 else self.escrow.database_b,
            "localStorageToken": self.escrow.local_storage,
            "preferenceTokenADigest": (
                None if ordinal == 3 else self.escrow.preference_a_digest
            ),
            "preferenceTokenBDigest": (
                None if ordinal == 1 else self.escrow.preference_b_digest
            ),
            "databaseTokenADigest": (
                None if ordinal == 3 else self.escrow.database_a_digest
            ),
            "databaseTokenBDigest": (
                None if ordinal == 1 else self.escrow.database_b_digest
            ),
...
class ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadServer(ThreadingHTTPServer):
    daemon_threads = True
...
class ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadRequestHandler(BaseHTTPRequestHandler):
    server: ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadServer
...
def create_server(
    ...
    *,
...
) -> ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadServer:
    ...
    artifacts = snapshot_regular_files(
        ...
        description=(
            "outer-reload profile "
            "Bookmark/Cookie/History/Database/LocalStorage artifacts"
        ),
    )
    ...
    server = ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadServer(
        (host, port), ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadRequestHandler
    )
...
def verify_server_delivery(server: ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadServer) -> None:
...
def expected_markers(ordinal: int, escrow: TokenEscrow) -> list[str]:
    if ordinal == 1:
        return [
            ...
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_A_WRITE_FLUSHED "
            f"sha256={escrow.preference_a_digest}",
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_MODEL_CLOSED",
            f"{M7_PREFS_MARKER_PREFIX}COOKIE_A_WRITE_FLUSHED "
            f"sha256={escrow.preference_a_digest}",
            f"{M7_PREFS_MARKER_PREFIX}COOKIE_BACKEND_CLOSED",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_A_WRITE_ACCEPTED",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_BACKEND_CLOSED",
            ...
            f"{M7_DATABASE_MARKER_PREFIX}SQLITE_WRITE_ACCEPTED "
            f"sha256={escrow.database_a_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}LEVELDB_WRITE_ACCEPTED "
            f"sha256={escrow.database_a_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}DATABASES_CLOSED "
            f"sha256={escrow.database_a_digest}",
            f"{M7_PREFS_MARKER_PREFIX}FENCE_OK "
            f"sha256={escrow.preference_a_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}FENCE_OK "
            f"sha256={escrow.database_a_digest}",
    ...
    if ordinal == 2:
        return [
            ...
            f"{M7_PREFS_MARKER_PREFIX}BROWSER_SMOKE_CLOSED",
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_A_READ_OK "
            f"sha256={escrow.preference_a_digest}",
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_B_WRITE_FLUSHED "
            f"sha256={escrow.preference_b_digest}",
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_MODEL_CLOSED",
            ...
            f"sha256={escrow.preference_b_digest}",
            f"{M7_PREFS_MARKER_PREFIX}COOKIE_BACKEND_CLOSED",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_A_READ_OK",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_B_WRITE_ACCEPTED",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_BACKEND_CLOSED",
            f"{M7_LOCAL_STORAGE_MARKER_PREFIX}READY",
            ...
            f"sha256={escrow.local_storage_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}READY",
            f"{M7_DATABASE_MARKER_PREFIX}SQLITE_READ_A_OK "
            f"sha256={escrow.database_a_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}LEVELDB_READ_A_OK "
            f"sha256={escrow.database_a_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}SQLITE_WRITE_ACCEPTED "
    ...
    if ordinal == 3:
        return [
            f"{M7_PREFS_MARKER_PREFIX}READY",
            f"{M7_PREFS_MARKER_PREFIX}READ_B_OK "
            f"sha256={escrow.preference_b_digest}",
            f"{M7_PREFS_MARKER_PREFIX}BROWSER_SMOKE_CLOSED",
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_B_READ_OK "
            f"sha256={escrow.preference_b_digest}",
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_CLEANUP_FLUSHED",
            f"{M7_PREFS_MARKER_PREFIX}BOOKMARK_MODEL_CLOSED",
            f"{M7_PREFS_MARKER_PREFIX}COOKIE_B_READ_OK "
            f"sha256={escrow.preference_b_digest}",
            f"{M7_PREFS_MARKER_PREFIX}COOKIE_BACKEND_CLOSED",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_A_READ_OK",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_B_READ_OK",
            f"{M7_PREFS_MARKER_PREFIX}HISTORY_BACKEND_CLOSED",
            f"{M7_LOCAL_STORAGE_MARKER_PREFIX}READY",
            f"{M7_LOCAL_STORAGE_MARKER_PREFIX}RENDERER_REOPEN_READ_OK "
            f"sha256={escrow.local_storage_digest}",
            ...
            f"sha256={escrow.local_storage_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}READY",
            f"{M7_DATABASE_MARKER_PREFIX}SQLITE_READ_B_OK "
            f"sha256={escrow.database_b_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}LEVELDB_READ_B_OK "
            f"sha256={escrow.database_b_digest}",
            f"{M7_DATABASE_MARKER_PREFIX}DATABASES_CLOSED "
            f"sha256={escrow.database_b_digest}",
            f"{M7_PREFS_MARKER_PREFIX}FENCE_OK "
...
def _validate_run(run: object, ordinal: int, escrow: TokenEscrow) -> tuple[str, str]:
    ...
    if (
        ...
        or run.get("bookmarkTransportModeFeatureDisabled") is not True
        ...
        or run.get("databaseLeaseReleasedMarkerObserved") is not True
    ...
    if any(
        (
            ...
            or M7_DATABASE_MARKER_PREFIX in line
    ...
    stderr_markers = [
        ...
        or line.startswith(M7_DATABASE_MARKER_PREFIX)
...
def _validate_token_evidence(value: object, ordinal: int, escrow: TokenEscrow) -> None:
    ...
    expected_database_a: str | None = (
        None if ordinal == 3 else escrow.database_a_digest
    )
    expected_database_b: str | None = (
        None if ordinal == 1 else escrow.database_b_digest
    )
    expected_distinct: bool | None = True if ordinal in (2, 3) else None
    if (
        evidence.get("algorithm") != "SHA-256"
        or evidence.get("preferenceA") != expected_a
        or evidence.get("preferenceB") != expected_b
        or evidence.get("databaseA") != expected_database_a
        or evidence.get("databaseB") != expected_database_b
...
def validate_outer_document_transitions(
    ...
    if (
        ...
        or len(
            {first.module_identity, second.module_identity, third.module_identity}
        )
        != 3
        or len(
            {first.loader_identity, second.loader_identity, third.loader_identity}
        )
        != 3
    ):
...
def validate_cdp_root_loaders(
    first: RootFrameIdentity,
    second: RootFrameIdentity,
    third: RootFrameIdentity,
) -> None:
    """Require one stable root frame and three globally distinct loaders."""

    if (
        not all(isinstance(value, RootFrameIdentity) for value in (first, second, third))
        or first.frame_id != second.frame_id
        or second.frame_id != third.frame_id
        or len({first.loader_id, second.loader_id, third.loader_id}) != 3
    ):
        raise M0Error("outer-reload DevTools loader transition is invalid")
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = (
        diagnostics_dir
        / "chrome-profile-bookmark-cookie-history-database-local-storage-outer-reload-m7-failure.json"
    )
    payload = {
        "schema_version": 1,
        "runner": "run_m7_chrome_profile_bookmark_cookie_history_database_local_storage_outer_reload_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "stage": stage,
        "nonclaims": FROZEN_NONCLAIMS,
        "failure": {
...
def _stop_server(
    server: ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadServer | None,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run three aggregate Bookmark + Cookie + History + Database + "
            "renderer LocalStorage Chrome Wasm documents separated by two "
            "real DevTools Page.reload calls."
        ),
        epilog=(
            "Build the normal artifact with: buildtools/linux64/gn gen "
            "out/wasm-chrome-m7-profile-bookmark-cookie-history-database-local-storage --args='"
            + DEFAULT_GN_ARGUMENTS
            + "' --fail-on-unused-args; autoninja -C "
            "out/wasm-chrome-m7-profile-bookmark-cookie-history-database-local-storage chrome_wasm"
        ),
    )
    ...
    parser.add_argument(
        "--timeout",
        type=parse_outer_reload_timeout,
        default=DEFAULT_TIMEOUT_SECONDS,
        help=(
            "single global deadline in seconds for browser startup, all three "
            "fresh documents, and both reloads (default: %(default)s)"
        ),
    )
    ...
    server: ChromeProfileBookmarkCookieHistoryDatabaseLocalStorageOuterReloadServer | None = None
    ...
    try:
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-server",
            daemon=True,
        )
        server_thread.start()
        ...
        outer_profile = tempfile.TemporaryDirectory(
            prefix="chromium-wasm-m7-profile-bookmark-cookie-history-database-local-storage-outer-reload-"
        )
        ...
        stderr_thread = threading.Thread(
            ...
            name=(
                "chromium-wasm-m7-profile-bookmark-cookie-history-database-local-storage-"
                "outer-reload-browser-stderr"
            ),
            daemon=True,
        )
        stderr_thread.start()
        ...
        if root_frame != second_root_frame:
            raise M0Error("outer-reload DevTools phase-two loader changed")
        ...
        third_root_frame = reload_outer_document(
            client,
            browser,
            browser_stderr,
            root_frame,
        ...
        stage = "validate-three-loader-transition"
        validate_cdp_root_loaders(
            first_root_frame, second_root_frame, third_root_frame
        )
        successful = True
        ...
        + json.dumps(
            {
                ...
                "otherDomStorageOriginsOrServicesProven": False,
                "artifactSourceProvenanceVerified": False,
                "nonclaims": FROZEN_NONCLAIMS,
                "tokenDigests": {
                    "databaseA": escrow.database_a_digest,
                    "databaseB": escrow.database_b_digest,
