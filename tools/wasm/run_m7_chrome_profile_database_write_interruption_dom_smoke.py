...
"""Observe one controlled post-LevelDB-log-Sync write interruption.

This deliberately exits nonzero even when it completes its intended three
document observation. It is not an M7 acceptance test, does not demonstrate
power-loss behavior, and must never be used as a persistence, directory
durability, outer-reload, or profile-gate success signal.
...
* document 1 cleanly seeds A using the dedicated diagnostic module;
...
SENTINEL = "CHROMIUM_WASM_M7_CHROME_PROFILE_DATABASE_WRITE_INTERRUPTION_DOM"
CASE = "chrome_profile_database_write_interruption_m7"
SCOPE = (
    "same-origin-three-outer-documents-chrome-wasm-m7-profile-database-post-"
    "leveldb-log-sync-interruption-diagnostic-only"
)
PRODUCT_MODULE_NAME = "chrome_wasm_m7_profile_database_write_interruption_diagnostic"
...
DEFAULT_OUT_DIR = Path("out/wasm-chrome-m7-profile-database-write-interruption")
DEFAULT_GN_ARGUMENTS = (
    'import("//out/wasm-chrome-m7-profile-database/args.gn") '
    "enable_chromium_wasm_m7_profile_database_test=true "
    + PRODUCT_GN_ENABLE_ARGUMENT
)
HOST_ROOT = "/__m7_chrome_profile_database_write_interruption__"
HOST_HTML_NAME = "chrome_wasm_profile_database_write_interruption_smoke.html"
HOST_JS_NAME = "chrome_wasm_profile_database_write_interruption_smoke.js"
...
M7_WRITE_INTERRUPTION_GN_ENABLE_ASSIGNMENT_RE = re.compile(
...
POST_SYNC_OBSERVATION_OUTCOMES = frozenset(
...
class PhaseResult:
    ...
    time_origin: float
    module_identity: str
    post_sync_observation: str | None
...
def new_capability_pair() -> tuple[str, str]:
    ...
    if not CAPABILITY_RE.fullmatch(result_token) or not CAPABILITY_RE.fullmatch(session):
        raise M0Error("write-interruption capability generation is invalid")
...
def _phase_mode(ordinal: int) -> str:
    ...
    if ordinal == 3:
        return "observe-leveldb-write-b"
    raise M0Error("write-interruption phase ordinal is invalid")
...
class WriteInterruptionSession:
    ...
    def __init__(self, result_token: str, session: str, escrow: TokenEscrow):
        if (
            ...
            raise M0Error("write-interruption session escrow is invalid")
    ...
    def _next_expected_document(self) -> int:
        ...
        raise ProtocolStateError("write-interruption bootstrap document conflict")
    ...
    def accept_bootstrap_document(self, session: str, evidence: DocumentEvidence) -> bool:
        ...
        if not _is_valid_document_evidence(evidence):
            raise ProtocolStateError("write-interruption bootstrap document is invalid")
        with self._lock:
    ...
    def acknowledge_bootstrap_document(self, session: str) -> bool:
        ...
        with self._lock:
            if self._pending_document is None:
                raise ProtocolStateError("write-interruption bootstrap acknowledgement conflict")
    ...
    def bootstrap_payload(self, session: str) -> dict[str, object] | None:
        ...
        with self._bootstrap_acknowledgement_gate:
            with self._lock:
                ...
                if len(candidates) != 1:
                    raise ProtocolStateError("write-interruption bootstrap state conflict")
    ...
    def arm_next_document(self, previous: int, time_origin: float) -> None:
        if previous not in (1, 2) or not isinstance(time_origin, (int, float)):
            raise M0Error("write-interruption next-document arm is invalid")
        ...
        with self._lock:
            ...
            if (
                ...
                raise ProtocolStateError("write-interruption next-document arm conflict")
    ...
    def document_evidence(self, ordinal: int) -> DocumentEvidence:
        with self._lock:
            ...
            if value is None:
                raise ProtocolStateError("write-interruption document evidence unavailable")
            return value
    ...
    def accept_result(self, result_token: str, ordinal: int) -> bool:
        ...
        with self._lock:
            if (
                ...
                raise ProtocolStateError("write-interruption result state conflict")
    ...
    def accept_ready(self, result_token: str, ordinal: int) -> bool:
        ...
        with self._lock:
            if (
                ...
                raise ProtocolStateError("write-interruption ready state conflict")
            self._ready_accepted.add(ordinal)
        return True


class ChromeProfileDatabaseWriteInterruptionServer(ThreadingHTTPServer):
    daemon_threads = True
    ...
    runner_source: bytes
    session: WriteInterruptionSession
...
class ChromeProfileDatabaseWriteInterruptionRequestHandler(BaseHTTPRequestHandler):
    server: ChromeProfileDatabaseWriteInterruptionServer
    ...
    def _conflict(self) -> None:
        self._send_bytes(
            ...
            b"write-interruption endpoint state conflict\n",
        )
    ...
    def _post_bootstrap_document(self, session: str) -> None:
        ...
        if evidence is None:
            self._send_bytes(
                ...
                "text/plain; charset=utf-8",
                b"invalid write-interruption bootstrap document\n",
            )
            return
        outcome = "accepted"
    ...
    def _post_result(self, result_token: str, ordinal: int) -> None:
        ...
        if value is None or not _is_receipt_identity(value, ordinal):
            self._send_bytes(
                HTTPStatus.BAD_REQUEST,
                "text/plain; charset=utf-8",
                b"invalid write-interruption result\n",
            )
            return
    ...
    def _post_ready(self, result_token: str, ordinal: int) -> None:
        ...
        if value is None or not _is_ready_identity(value, ordinal):
            self._send_bytes(
                HTTPStatus.BAD_REQUEST,
                "text/plain; charset=utf-8",
                b"invalid write-interruption ready\n",
            )
            return
        outcome = "accepted"
        with self.server.receipt_lock:
...
def _require_product_module_name(module_name: object, boundary: str) -> str:
    if not isinstance(module_name, str) or module_name != PRODUCT_MODULE_NAME:
        raise M0Error(f"write-interruption {boundary} module is invalid")
    return module_name


def validate_m7_output_configuration(args_gn: bytes) -> None:
    """Require exactly the distinct post-Sync diagnostic configuration."""

    try:
        text = args_gn.decode("utf-8").replace("\r\n", "\n")
    except UnicodeDecodeError as exc:
        raise M0Error("write-interruption args.gn is not UTF-8") from exc
...
def create_server(
    ...
    *,
...
) -> ChromeProfileDatabaseWriteInterruptionServer:
    ...
    artifacts = snapshot_regular_files(
        ...
        description="write-interruption profile database artifacts",
    )
    args_gn = snapshot_regular_file(
        out_dir / "args.gn",
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="write-interruption selected args.gn",
    )
    ...
    host_snapshots = snapshot_regular_files(
        ...
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="write-interruption host resources",
    )
    runner_source = snapshot_regular_file(
        runner_source_path or Path(__file__),
        maximum_bytes=MAX_SNAPSHOT_BYTES,
        description="write-interruption runner source",
    )
    server = ChromeProfileDatabaseWriteInterruptionServer(
        (host, port), ChromeProfileDatabaseWriteInterruptionRequestHandler
    )
    ...
    server.session = WriteInterruptionSession(result_token, session, escrow)
    return server
...
def artifact_identity(
    ...
    *,
    ...
    if server.module_name != module_name:
        raise M0Error("write-interruption artifact module disagrees with server")
    return {
...
def toolchain_manifest_versions(manifest: dict[str, Any]) -> dict[str, str]:
    try:
        ...
        raise M0Error("write-interruption manifest lacks version metadata") from exc
    if not all(
        isinstance(value, str) and GIT_REVISION_RE.fullmatch(value)
        for value in versions.values()
    ):
        raise M0Error("write-interruption manifest version metadata is invalid")
    return versions
...
def smoke_url(
    ...
    *,
    ...
    if server.module_name != module_name:
        raise M0Error("write-interruption URL module disagrees with server")
    if not server.session.matches_result_token(result_token) or not server.session.matches_session(
        session
    ):
        raise M0Error("write-interruption URL capability disagrees with server")
    if not isinstance(timeout_seconds, (int, float)) or isinstance(timeout_seconds, bool):
        raise M0Error("write-interruption URL timeout is invalid")
...
def _require_exact_fields(value: object, fields: frozenset[str], name: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != fields:
        raise M0Error(f"write-interruption {name} schema is invalid")
    return value
...
def _require_equal(value: dict[str, Any], field: str, expected: object) -> None:
    if not _exact_json_equal(value.get(field), expected):
        raise M0Error(f"write-interruption result {field} is invalid")
...
def _validate_byte_identity(value: object, description: str) -> None:
    ...
    if (
        ...
        raise M0Error(f"write-interruption {description} is invalid")
...
def _validate_artifact_identity(value: object, expected: dict[str, object]) -> None:
    ...
    if value != expected:
        raise M0Error("write-interruption artifact identity is invalid")
...
def _validate_capture_harness_identity(value: object, expected: dict[str, object]) -> None:
    ...
    if value != expected:
        raise M0Error("write-interruption capture harness is invalid")
...
def expected_markers(ordinal: int, escrow: TokenEscrow, outcome: str | None = None) -> list[str]:
    ...
    if ordinal == 3 and outcome in POST_SYNC_OBSERVATION_OUTCOMES:
        return [
            f"{M7_DATABASE_MARKER_PREFIX}READY",
...
def _validate_token_evidence(value: object, ordinal: int, escrow: TokenEscrow) -> None:
    ...
    if (
        ...
        raise M0Error("write-interruption token evidence is invalid")
...
def _validate_document(
    ...
    if (
        ...
        raise M0Error("write-interruption document is invalid")
    actual = DocumentEvidence(document["navigationType"], float(time_origin))
    if actual != expected:
        raise M0Error("write-interruption document disagrees with bootstrap")
    return actual
...
def _validate_run(value: object, ordinal: int, escrow: TokenEscrow) -> tuple[str, str | None]:
    ...
    if (
        ...
        raise M0Error("write-interruption run identity is invalid")
    ...
    if ordinal == 2:
        if (
            ...
            or run.get("postSyncObservation") is not None
            or run.get("settleWindowMs") != INTERRUPTION_SETTLE_MS + FINAL_QUIESCENCE_MS
        ):
            raise M0Error("write-interruption interrupted run is invalid")
        return module_identity, None

    outcome: str | None = None
    if ordinal == 3:
        outcome = run.get("postSyncObservation")
        if outcome not in POST_SYNC_OBSERVATION_OUTCOMES:
            raise M0Error("write-interruption post-Sync observation is invalid")
    elif run.get("postSyncObservation") is not None:
        raise M0Error("write-interruption seed post-Sync observation is invalid")
    if (
        ...
        raise M0Error("write-interruption clean diagnostic run is invalid")
...
def _validate_bridge(value: object, ordinal: int) -> None:
    ...
    if (
        ...
        raise M0Error("write-interruption bridge is invalid")
...
def _validate_final_quiescence(value: object, ordinal: int) -> None:
    ...
    if (
        ...
        raise M0Error("write-interruption final quiescence state is invalid")
    callback_fields = (
        "callbacksAtStart",
        "callbacksAtEnd",
        "callbacksAtPreUploadCheck",
    ...
    if any(
        ...
        raise M0Error("write-interruption final quiescence counters are invalid")
    if len({quiescence[field] for field in callback_fields}) != 1 or len(
        {quiescence[field] for field in process_exit_fields}
    ) != 1:
        raise M0Error("write-interruption final quiescence is not quiet")
    if quiescence["processExitReportsAtStart"] != expected_process_exits:
        raise M0Error("write-interruption final quiescence exit count is invalid")
...
def _validate_host_boundary(value: object) -> None:
    ...
    if any(field_value is not False for field_value in boundary.values()):
        raise M0Error("write-interruption host crossed a prohibited boundary")
...
def validate_phase_result(
    ...
    *,
    ...
    """Validate one fixed, non-gating observation receipt."""

    if _contains_prohibited_strings(
        result, (escrow.token_a, escrow.token_b, result_token, session)
    ):
        raise M0Error("write-interruption receipt contains an opaque value")
...
def validate_ready_receipt(ready: dict[str, Any], *, expected: PhaseResult) -> None:
    ...
    if (
        ...
        raise M0Error("write-interruption ready receipt is invalid")
...
def validate_three_document_transition(
    ...
    if (
        ...
        or third.post_sync_observation not in POST_SYNC_OBSERVATION_OUTCOMES
    ):
        raise M0Error("write-interruption three-document transition is invalid")
...
def verify_server_delivery(server: ChromeProfileDatabaseWriteInterruptionServer) -> None:
    ...
    for path, contents, content_type in expected:
        ...
        try:
            ...
            if response.status != HTTPStatus.OK:
                raise M0Error("write-interruption snapshot request failed")
            ...
            for name, expected_value in _expected_headers(content_type).items():
                ...
                if actual != expected_value:
                    raise M0Error("write-interruption snapshot response header is invalid")
            count, digest = _stream_response_digest(response)
            if count != len(contents) or digest != hashlib.sha256(contents).hexdigest():
                raise M0Error("write-interruption snapshot body changed")
        finally:
            connection.close()
...
def _wait_for_receipt(
    ...
    while True:
        ...
        if remaining <= 0:
            raise M0Error(
                f"write-interruption {kind} timed out "
        ...
        try:
            ...
            if browser.poll() is not None:
                raise M0Error(
                    f"write-interruption browser exited before {kind} "
                    f"({len(browser_stderr)} browser stderr lines suppressed)"
                )
            continue
        if receipt_ordinal != ordinal:
            raise M0Error("write-interruption receipt order is invalid")
        return receipt
...
def _root_frame_identity(value: object) -> RootFrameIdentity:
    if not isinstance(value, dict):
        raise M0Error("write-interruption DevTools frame tree is invalid")
    ...
    if (
        ...
        raise M0Error("write-interruption DevTools root frame is invalid")
...
def _root_reload_event(
    ...
    *,
    ...
    if not isinstance(params, dict):
        raise M0Error("write-interruption DevTools navigation event is invalid")
    ...
    if "parentId" in frame:
        raise M0Error("write-interruption DevTools root frame changed parent")
    loader_id = frame.get("loaderId")
    url = frame.get("url")
    if not isinstance(loader_id, str) or not loader_id:
        raise M0Error("write-interruption DevTools navigation loader is invalid")
    if loader_id == baseline.loader_id:
        return None
    if not isinstance(url, str) or not url.startswith(expected_page_url_prefix):
        raise M0Error("write-interruption DevTools navigation URL is invalid")
...
def wait_for_root_reload_navigation(
    ...
    while True:
        ...
        if remaining <= 0:
            raise M0Error(
                "write-interruption DevTools navigation timed out "
                f"({len(browser_stderr)} browser stderr lines suppressed)"
            )
        if browser.poll() is not None:
            raise M0Error(
                "write-interruption browser exited before DevTools navigation "
                f"({len(browser_stderr)} browser stderr lines suppressed)"
            )
...
def write_failure_diagnostics(
    ...
    *,
    ...
    path = diagnostics_dir / "chrome-profile-database-write-interruption-m7-failure.json"
    payload = {
        "schema_version": 1,
        "runner": "run_m7_chrome_profile_database_write_interruption_dom_smoke.py",
        "case": CASE,
        "scope": SCOPE,
        "m7GateComplete": False,
        "stage": stage,
...
def _stop_server(
    server: ChromeProfileDatabaseWriteInterruptionServer | None,
    ...
    if server_thread_started and server_thread is not None:
        ...
        if server_thread.is_alive():
            raise M0Error("write-interruption server did not stop")


def _observation_summary(outcome: str) -> dict[str, object]:
    """Return bounded output that cannot be mistaken for M7 success."""

    if outcome not in POST_SYNC_OBSERVATION_OUTCOMES:
        raise M0Error("write-interruption observation summary is invalid")
    return {
        "case": CASE,
        "controlledWriteInterruptionObservation": True,
        "documents": 3,
        "m7GateComplete": False,
        "outerReloadProven": False,
        ...
        "crashBehaviorProven": False,
        "postSyncValueObservation": outcome,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Observe a controlled Chrome Wasm post-LevelDB-log-Sync interruption; "
            "this diagnostic always exits nonzero."
        ),
        epilog=(
            "Build the isolated artifact with: buildtools/linux64/gn gen "
            "out/wasm-chrome-m7-profile-database-write-interruption --args='"
            + DEFAULT_GN_ARGUMENTS
            + "' --fail-on-unused-args; autoninja -C "
            "out/wasm-chrome-m7-profile-database-write-interruption chrome_wasm"
        ),
    )
    ...
    server: ChromeProfileDatabaseWriteInterruptionServer | None = None
    ...
    try:
        ...
        server_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-profile-database-write-interruption-server",
            daemon=True,
        )
        server_thread.start()
        ...
        outer_profile = tempfile.TemporaryDirectory(
            prefix="chromium-wasm-m7-profile-database-write-interruption-"
        )
        ...
        browser_stderr_thread = threading.Thread(
            ...
            name="chromium-wasm-m7-profile-database-write-interruption-browser-stderr",
            daemon=True,
        )
        ...
        stage = "arm-post-sync-observation-document"
        server.session.arm_next_document(2, second.time_origin)
        stage = "reload-to-post-sync-observation-document"
        root_frame = reload_outer_document(
            client, browser, browser_stderr, root_frame, expected_page_url_prefix, deadline
        )

        stage = "wait-post-sync-observation-result"
        third_result = wait_for_phase_result(browser, browser_stderr, server, 3, deadline)
        result_ordinals_received.add(3)
        stage = "validate-post-sync-observation-result"
        ...
        observation = _observation_summary(third.post_sync_observation or "")
    ...
    # A completed observation is deliberately still nonzero. It is neither an
    # M7 gate nor a positive durability, persistence, or reload result.
    return 1
