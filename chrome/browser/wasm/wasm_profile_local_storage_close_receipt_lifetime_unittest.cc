...
using ReceiptLifetime = WasmProfileLocalStorageCloseReceiptLifetime;
...
// Mirrors the production participant's ownership rule: owner loss moves the
// controller-containing State into process-lifetime quarantine rather than
// destroying its still-outstanding admission.
class TestOwner {
 public:
  ...
  void ReceiveExactCloseReceipt() { state_->ReceiveExactCloseReceipt(); }
  void FailBeforeCloseReceipt() { state_->FailBeforeCloseReceipt(); }
 ...
 private:
  class State {
   public:
    ...
    void Cancel() { lifetime_.Cancel(); }
    bool IsActive() const { return lifetime_.IsActive(); }
    bool HasOutstandingAdmission() const {
      return lifetime_.HasOutstandingAdmission();
    }

    void ReceiveExactCloseReceipt() {
      lifetime_.CompleteAfterExactCloseReceipt(
          base::BindOnce(&State::Cleanup, base::Unretained(this)));
    }

    void FailBeforeCloseReceipt() {
      lifetime_.FailBeforeExactCloseReceipt(
          base::BindOnce(&State::Cleanup, base::Unretained(this)));
    }

    void PrepareForOwnerQuarantine() {
      lifetime_.Cancel();
      if (!lifetime_.HasExactCloseReceipt()) {
        FailBeforeCloseReceipt();
        return;
      }
      Cleanup();
    }
...
TEST(WasmProfileLocalStorageCloseReceiptLifetimeTest,
     ExactReceiptAndPostedDeliveryOwnAdmission) {
  ...
  owner.ReceiveExactCloseReceipt();
...
TEST(WasmProfileLocalStorageCloseReceiptLifetimeTest,
     FailureBeforeReceiptCleansThenQuarantinesOutstandingAdmission) {
  base::test::TaskEnvironment task_environment;
  Lifecycle lifecycle;
  std::optional<Lifecycle::ProfileIOHold> hold =
      lifecycle.TryAcquireProfileIO();
...
TEST(WasmProfileLocalStorageCloseReceiptLifetimeTest,
     OwnerDestructionBeforeQueuedFailureDeliveryCannotAbandonOrUAF) {
  base::test::TaskEnvironment task_environment;
  Lifecycle lifecycle;
  std::optional<Lifecycle::ProfileIOHold> hold =
      lifecycle.TryAcquireProfileIO();
