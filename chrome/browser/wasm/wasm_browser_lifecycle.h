...
class WasmBrowserLifecycle final {
 public:
  ...
  // ordinary-JavaScript expression in the lifecycle-owned page and checks
  // closures, classes, Promise microtasks, BigInt, and typed arrays. It does
  // not inspect WebAssembly or expose a host-selected URL, expression, or
  // protocol command.
  void StartPageJavaScriptSemanticsDevToolsProtocolSmoke();

  // Starts the separate, test-only fixed proof that awaits one literal
  // ordinary-JavaScript async rejection, catch, and finally ordering path in
  // the lifecycle-owned page. It does not inspect WebAssembly or expose a
  ...
  // Starts the separate, test-only fixed proof that imports a one-entry table,
  // grows it once to two entries through JavaScript, initializes its new entry,
  // and calls it indirectly from Wasm. It does not exercise the Wasm
  // table.grow opcode, broader reference types, exceptions, or threads.
  void StartPageWebAssemblyTableGrowthDevToolsProtocolSmoke();

  // Starts the separate, test-only fixed proof that constructs a JavaScript
  // WebAssembly.Exception for one imported tag, throws it through one imported
  // JavaScript function, and catches it in Wasm. It does not exercise Wasm
  // throw, payload, rethrow, catch-all, Wasm-to-JavaScript escape, or threads.
  void StartPageWebAssemblyExceptionDevToolsProtocolSmoke();

  // Starts the separate, test-only fixed proof that imports one bounded
  // WebAssembly.Memory, grows it once through Wasm's memory.grow opcode, and
  // verifies its returned old page count, buffer replacement, and final size.
  // It does not exercise shared or multiple memories, data exchange, tables,
  ...
  // catches, rethrow, catch-all, throw_ref, tables, memories, or threads.
  void StartPageWebAssemblyWasmThrowDevToolsProtocolSmoke();

  // Starts the separate, test-only fixed proof that imports one i32-payload
  // tag, executes Wasm throw with payload 42, and validates JavaScript tag
  // identity and payload extraction. It does not exercise other payload types,
