...
void VerifyPersistentPrefsAndReplyOnFileSequence(
  ...
  bool readback_succeeded =
    ...
    defined(CHROME_WASM_M7_PERSISTENT_DEFAULT_PARTITION_POLICY_PROBE) || \
    defined(CHROME_WASM_M7_PERSISTENT_DEFAULT_PARTITION_SHUTDOWN_PROBE)
  // JsonPrefStore has verified the bytes; now require the profile directory
  // namespace itself to acknowledge fsync before this operation can become a
  // clean profile-I/O receipt. This prevents a successful file write from
  // hiding an ENOTSUP directory backend at the Chrome handoff boundary.
  if (readback_succeeded) {
    readback_succeeded = chrome::SyncWasmProfileStorageDirectory();
  }
#endif
...
bool WasmProfile::ShouldUseInMemoryDefaultStoragePartition() {
  ...
  // The source-selected shutdown probe fails closed unless its first default
  // configuration derivation creates the real disk-backed default partition.
  // Chromium may later derive that same configuration for SiteInfo/frame-host
  // bookkeeping; the renderer witness instead validates actual partition
  // identity and sole-map/no-create boundaries. Normal Chrome remains volatile
  // and this artifact always chooses failure retirement after map drop.
  chrome::RecordWasmPersistentDefaultPartitionShutdownProbePolicyQuery(this);
  return false;
#else
